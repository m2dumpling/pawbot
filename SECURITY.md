...
If you discover a security vulnerability in pawbot, please report it by:

1. **DO NOT** open a public GitHub issue
2. Create a private security advisory through the repository's configured security contact
3. Include:
...
- Config files and API keys (`~/.pawbot/config.json`) → **hidden** (masked by tmpfs)
...
- ✅ Run pawbot with a dedicated user account
...
**WhatsApp:**
- Keep the neonize session database under `~/.pawbot/whatsapp-auth` secure (mode 0700).
- Use `pawbot channels login whatsapp --force` to remove and recreate the local session database when rotating linked devices.
...
- Subscribe to security advisories for pawbot and its dependencies
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.pawbot
   chmod 600 ~/.pawbot/config.json
   chmod 700 ~/.pawbot/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.pawbot/logs/pawbot.log
   ```
...
- **Chat history is stored locally** - protect the `~/.pawbot` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.pawbot/logs/pawbot.log
   ```
...
- Private security advisories: use the repository's configured security reporting mechanism.
- Release notes: see the repository's release page and [`CHANGELOG.md`](CHANGELOG.md).
