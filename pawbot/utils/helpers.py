...
_TOOL_RESULTS_DIR = ".pawbot/tool-results"
...
def estimate_message_tokens(message: dict[str, Any], model: str | None = None) -> int:
    ...
    Delegates to ``pawbot.agent.token_estimation`` (model-aware tiktoken with
    structural overhead, ADR-002). Kept here for import compatibility.
    """
    from pawbot.agent.token_estimation import count_message_tokens

    return count_message_tokens(message, model)
...
def load_bundled_template(template_name: str) -> str | None:
    """Read a bundled template file from the pawbot package."""
    from importlib.resources import files as pkg_files

    with suppress(Exception):
        tpl = pkg_files("pawbot") / "templates" / template_name
        if tpl.is_file():
