...
from pawbot.agent.plugins import (
    AgentPlugin,
...
from pawbot.config.loader import load_config, resolve_config_env_vars, save_config
...
if TYPE_CHECKING:
    from pawbot.webui.settings_services import WebUISettingsConfig
...
def _custom_manifest(name: str, cfg: MCPServerConfig) -> dict[str, Any]:
    ...
    return app_manifest(
        app_id=name,
        display_name=name,
        description="Custom MCP server from pawbot config.",
        category="custom",
        source="mcp-custom",
        brand_color="#64748B",
        capabilities=[
