...
from pawbot.agent.tools.image_generation import request_image_generation_reload
from pawbot.agent.tools.mcp_oauth import MCP_OAUTH_CALLBACK_PATH
from pawbot.api.runtime import ApiRuntime, api_runtime_paths
...
from pawbot.pairing import approve_code, deny_code, list_pending
from pawbot.webui import settings_capabilities as capability_domain
from pawbot.webui import settings_contracts as contracts
from pawbot.webui import settings_models as model_domain
from pawbot.webui import settings_system as system_domain
from pawbot.webui.cli_apps_api import cli_apps_action, cli_apps_payload
from pawbot.webui.http_utils import http_response as _http_response
from pawbot.webui.http_utils import is_local_browser_request as _is_local_browser_request
from pawbot.webui.mcp_oauth_api import McpOAuthManager
from pawbot.webui.mcp_presets_api import (
    ensure_mcp_oauth_server,
    mcp_presets_settings_action,
)
from pawbot.webui.pawbot_features_api import (
    pawbot_feature_instance_target,
    pawbot_features_action,
    pawbot_features_payload,
)
from pawbot.webui.settings_api import (
...
from pawbot.webui.version_check import check_for_update

_WEBUI_MUTATION_PAYLOAD_ATTR = "_pawbot_webui_mutation_payload"
_WEBUI_MUTATION_REQUEST_ATTR = "_pawbot_webui_mutation_request"
...
_MODEL_ROUTES = {
    ...
    "/api/settings/reasoning-effort-values": "reasoning-effort-values",
...
_SYSTEM_ROUTES = {
    ...
    "/api/settings/pawbot-features": "features-list",
    "/api/settings/pawbot-features/enable": "features-enable",
    "/api/settings/pawbot-features/disable": "features-disable",
...
_SETTINGS_MUTATION_PATHS = frozenset({
    ...
    "/api/settings/pawbot-features/enable",
    "/api/settings/pawbot-features/disable",
...
class WebUISettingsRouter:
    ...
    def _model_operations(self) -> model_domain.ModelSettingsOperations:
        return model_domain.ModelSettingsOperations(
            ...
            reasoning_effort_values=model_domain.reasoning_effort_values_payload,
    ...
    def _system_operations(self) -> system_domain.SystemSettingsOperations:
        return system_domain.SystemSettingsOperations(
            ...
            pawbot_features_payload=pawbot_features_payload,
            pawbot_features_action=pawbot_features_action,
            pawbot_feature_instance_target=pawbot_feature_instance_target,
    ...
    async def _reload_mcp_runtime(self) -> dict[str, Any]:
        if self._mcp_reload is None:
            return {
                "ok": False,
                "message": "MCP runtime reload is unavailable. Restart pawbot to apply changes.",
        ...
        try:
            ...
            return {
                "ok": False,
                "message": "MCP hot reload timed out. Restart pawbot to pick up changes.",
                "requires_restart": True,
            }
            ...
            return {
                "ok": False,
                "message": "MCP hot reload failed. Restart pawbot to pick up changes.",
                "requires_restart": True,
                "error": str(exc),
            }
    ...
    def _pawbot_features_payload(self) -> dict[str, Any]:
        return pawbot_features_payload(config_path=self.settings.config.path)

    def _pawbot_features_action(
        self,
        action: str,
        query: QueryParams,
        *,
    ...
    def _handle_mcp_oauth_callback(self, request: WsRequest) -> Response:
        ...
        return self._mcp_oauth_callback_page(
            ok=True,
            message=f"Authorization received for {name}. Return to pawbot to finish connecting.",
        )
