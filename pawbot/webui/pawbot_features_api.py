"""Pawbot optional feature helpers for WebUI Settings."""
...
from pawbot.webui.http_utils import query_first

QueryParams = dict[str, list[str]]


def pawbot_features_payload(*, config_path: Path | None = None) -> dict[str, Any]:
...
def pawbot_feature_instance_target(query: QueryParams) -> str | None:
...
def pawbot_features_action(
    ...
    *,
    ...
    instance_id = pawbot_feature_instance_target(query)
    if not name:
    ...
    if action == "disable":
        ...
        if plugin is not None and "always_enabled" in plugin.capabilities:
            raise OptionalFeatureError(
                ...
                f"Use `pawbot plugins disable {name}` from a terminal if you need to disable it.",
                status=400,
            )
