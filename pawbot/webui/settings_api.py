...
from pawbot.webui import settings_capabilities as capabilities
from pawbot.webui import settings_contracts as contracts
from pawbot.webui import settings_models as models
from pawbot.webui import settings_system as system
from pawbot.webui.settings_contracts import QueryParams, WebUISettingsError
from pawbot.webui.workspaces import write_webui_default_access_mode
...
def update_observability_settings(
    ...
    *,
    ...
    config = _load_settings_config(config_path)
    changed = capabilities.update_observability_settings(config, query)
    if changed:
        _save_settings_config(config, config_path)
    return settings_payload(
