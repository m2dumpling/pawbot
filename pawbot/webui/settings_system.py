...
from pawbot.channels._setup import channel_setup_spec
from pawbot.channels.connect import ChannelConnectError
...
from pawbot.llm_usage import llm_usage_payload
from pawbot.optional_features import OptionalFeatureError, with_channel_runtime_status
from pawbot.security.workspace_access import workspace_sandbox_status
from pawbot.webui.settings_capabilities import network_safety_payload
...
class SystemSettingsOperations:
    ...
    pawbot_features_payload: SettingsOperation
    pawbot_features_action: SettingsOperation
    pawbot_feature_instance_target: SettingsOperation
...
def docs_payload(version: str) -> dict[str, Any]:
    ...
    configured_base = os.environ.get("PAWBOT_DOCS_URL", "").strip().rstrip("/")
    if not configured_base:
        return {
            "version": selected_version,
            "base_url": _LOCAL_DOCS_BASE_URL,
            "chat_apps_url": _LOCAL_CHAT_APPS_DOCS_URL,
            "latest_url": _LOCAL_DOCS_BASE_URL,
        }
    return {
        "version": selected_version,
        "base_url": f"{configured_base}/{selected_version}",
        "chat_apps_url": f"{configured_base}/{selected_version}/chat-apps",
        "latest_url": f"{configured_base}/latest",
    }
...
class SystemSettingsHandler:
    ...
    async def _features(
        ...
        try:
            payload = await asyncio.to_thread(
                operations.pawbot_features_payload,
                config_path=self.settings.config.path,
            )
        except Exception:
            self.logger.exception("failed to load pawbot features")
            return SettingsRouteResult.failure(500, "failed to load pawbot features")
        return SettingsRouteResult.success(
            self._with_channel_runtime_status(payload, operations)
        )

    def _pawbot_features_payload(
        self,
        operations: SystemSettingsOperations,
    ) -> dict[str, Any]:
        return operations.pawbot_features_payload(config_path=self.settings.config.path)
    ...
    def _pawbot_features_action(
        ...
        *,
        ...
        return self.settings.mutate(
            operations.pawbot_features_action,
            action,
            query,
    ...
    async def _apply_feature_runtime_change(
        ...
        if not name:
            return payload
        try:
            instance_id = operations.pawbot_feature_instance_target(query)
    ...
    async def _channel_configure(
        ...
        if not enable:
            features = await asyncio.to_thread(
                self._pawbot_features_payload,
                operations,
            )
            payload["pawbot_features"] = self._with_channel_runtime_status(
                features,
                operations,
            )
            return SettingsRouteResult.success(
                ...
                decorate_restart=True,
                restart_section="runtime",
                restart_payload_key="pawbot_features",
            )
    ...
    async def _with_channel_connect_success(
        ...
        try:
            ...
            features = self.feature_runtime_fallback(
                self._pawbot_features_payload(operations),
                message=(
        ...
        updated = dict(payload)
        updated["pawbot_features"] = self._with_channel_runtime_status(
            features,
            operations,
        )
        return updated
