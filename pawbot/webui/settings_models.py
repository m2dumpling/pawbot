...
from pawbot.config.loader import resolve_config_env_vars
from pawbot.config.schema import Config, FallbackCandidate, ModelPresetConfig, ProviderConfig
from pawbot.providers import registry
from pawbot.providers.image_generation import get_image_gen_provider
from pawbot.providers.oauth_guidance import OAUTH_CLI_KIT_MISSING_MESSAGE
from pawbot.providers.registry import PROVIDERS, create_dynamic_spec, find_by_name
from pawbot.webui.settings_contracts import (
    QueryParams,
    SettingsRequest,
    SettingsRouteResult,
    WebUISettingsError,
...
if TYPE_CHECKING:
    from pawbot.webui.settings_services import (
...
class ModelSettingsOperations:
    ...
    reasoning_effort_values: SettingsOperation
...
def oauth_provider_status(spec: Any) -> dict[str, Any]:
    ...
    if spec.name == "github_copilot":
        try:
            from pawbot.providers.github_copilot_provider import get_github_copilot_login_status
        except Exception:
            return {
                "configured": False,
                "account": None,
    ...
    if spec.name == "xai_grok":
        try:
            from pawbot.providers.xai_oauth import get_xai_oauth_login_status
        except Exception:
            return {
                "configured": False,
                "account": None,
...
def _model_id_from_row(row: Any) -> str | None:
    ...
    for key in ("id", "name", "model", "model_id", "model_name"):
...
def _model_metadata_mappings(row: Any) -> list[dict[str, Any]]:
    """Return the top-level and common nested model metadata mappings.

    OpenAI-compatible gateways are not fully consistent here: some put
    capability fields directly on the model row, while others nest them under
    ``capabilities`` or ``metadata``.  Keeping this normalization at the
    boundary lets the rest of the settings code stay provider-agnostic.
    """
    if not isinstance(row, dict):
        return []
    row_mapping = cast(dict[str, Any], row)
    mappings = [row_mapping]
    for key in ("capabilities", "metadata", "model_info", "limits", "reasoning", "thinking"):
        nested = row_mapping.get(key)
        if isinstance(nested, dict):
            mappings.append(cast(dict[str, Any], nested))
    return mappings
...
def _normalize_reasoning_effort_values(value: Any) -> list[str] | None:
    if isinstance(value, str):
    ...
    for raw_value in raw_values:
        ...
        if normalized in {"", "default", "none", "off", "disabled"}:
            normalized = ""
...
def _model_reasoning_effort_values(row: Any) -> list[str] | None:
    for mapping in _model_metadata_mappings(row):
        for key in (
            ...
            values = _normalize_reasoning_effort_values(mapping.get(key))
            if values is not None:
                return values
    return None


def _model_capability_spec(spec: Any | None, model_id: str) -> Any | None:
    """Find curated metadata even when a model is reached through a gateway."""
    return registry.model_capability_for(
        getattr(spec, "name", "") if spec is not None else "",
        model_id,
    )


def _model_row_payload(row: Any, *, spec: Any | None = None) -> dict[str, Any] | None:
    model_id = _model_id_from_row(row)
    if not model_id:
        return None
    capability = _model_capability_spec(spec, model_id)
    label: str | None = None
    ...
    if label is None and capability is not None and capability.label:
    ...
    if reasoning_effort_values:
        payload["reasoning_effort_values"] = reasoning_effort_values
    return payload


def _extract_model_rows(body: Any, *, spec: Any | None = None) -> list[dict[str, Any]]:
    if isinstance(body, dict):
        body_mapping = cast(dict[str, Any], body)
        raw_rows = (
            body_mapping.get("data")
            or body_mapping.get("models")
            or body_mapping.get("items")
        )
    else:
        raw_rows = body
    ...
    for raw_row in cast(list[object], raw_rows):
        row = _model_row_payload(raw_row, spec=spec)
...
def provider_models_payload(
    ...
    *,
        ...
        rows = [
            {
                ...
                **(
                    ...
                    if model.reasoning_effort_values
                    else {}
                ),
            }
    ...
    try:
        ...
        rows = _extract_model_rows(response.json(), spec=spec)
...
def _parse_context_window_tokens(value: str | None) -> int | None:
    ...
    if parsed <= 0:
        raise WebUISettingsError("context_window_tokens must be greater than zero")
    return parsed
...
def reasoning_effort_values_for(provider_name: str, model: str) -> list[str]:
    ...
    return registry.reasoning_effort_values_for(provider_name, model)
...
def reasoning_effort_values_payload(
    ...
    return {
        "provider": provider,
        "model": model,
        "values": reasoning_effort_values_for(provider, model),
    }
...
def update_agent_model_settings(
    ...
    *,
    ...
    if (
        ...
        defaults.context_window_mode = "manual"
        changed = True
    elif context_window_tokens is not None:
        if defaults.context_window_mode != "manual":
            defaults.context_window_mode = "manual"
            changed = True
    ...
    if "reasoning_effort" in query or "reasoningEffort" in query:
        ...
        if defaults.reasoning_effort != reasoning_effort:
...
def create_model_configuration(
    ...
    *,
    ...
    config.model_presets[name] = ModelPresetConfig(
        ...
        context_window_mode=(
            "manual" if context_window_tokens is not None else base.context_window_mode
        ),
...
def update_model_configuration(
    ...
    *,
    ...
    if (
        ...
        preset.context_window_mode = "manual"
        changed = True
    elif context_window_tokens is not None:
        if preset.context_window_mode != "manual":
            preset.context_window_mode = "manual"
            changed = True
...
def migrate_model_configurations(config: Config) -> bool:
    ...
    if not defaults.model_preset or defaults.model_preset == "default":
        ...
        config.model_presets[name] = ModelPresetConfig(
            ...
            context_window_mode=primary.context_window_mode,
    ...
    for fallback in defaults.fallback_models:
        ...
        config.model_presets[name] = ModelPresetConfig(
            ...
            context_window_mode=(
                "manual" if fallback.context_window_tokens is not None else primary.context_window_mode
            ),
            temperature=(
...
def login_oauth_provider(
    ...
    *,
    ...
    if spec.name == "openai_codex":
        try:
            from pawbot.providers.openai_codex_oauth import start_openai_codex_oauth_login
        except ImportError:
    ...
    if spec.name == "github_copilot":
        try:
            from pawbot.providers.github_copilot_provider import (
    ...
    if spec.name == "xai_grok":
        from pawbot.providers.xai_oauth import start_xai_oauth_login
...
def complete_oauth_provider(
    ...
    *,
    ...
    try:
        if spec.name == "openai_codex":
            from pawbot.providers.openai_codex_oauth import (
            ...
            from pawbot.providers.xai_oauth import complete_xai_oauth_login
...
def logout_oauth_provider(
    ...
    *,
    ...
    if spec.name == "openai_codex":
        ...
        from pawbot.providers.xai_oauth import logout_xai_oauth
...
class ModelSettingsHandler:
    ...
    async def handle(
        ...
        try:
            ...
            if action == "reasoning-effort-values":
                payload = await asyncio.to_thread(
                    self.settings.read,
                    operations.reasoning_effort_values,
                    request.query,
                )
                return SettingsRouteResult.success(payload)
