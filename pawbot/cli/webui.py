...
def webui(
    ...
    background: bool = typer.Option(
        False,
        "--background",
        help="Deprecated; use `pawbot gateway --background`",
    ),
    ...
    if background:
        ...
        command = ["pawbot", "gateway", "--background", "--config", str(config_path)]
        if workspace:
            command.extend(
                ["--workspace", str(Path(workspace).expanduser().resolve(strict=False))]
            )
        console.print(
            "[red]`pawbot webui --background` no longer owns gateway lifecycle.[/red]"
        )
        ...
        console.print(
            "  [cyan]pawbot webui --config " + shlex.quote(str(config_path)) + "[/cyan]"
        )
        raise typer.Exit(1)
    created_config = not config_path.exists()
    if created_config:
        console.print(f"[yellow]No config found at {config_path}.[/yellow]")
        _confirm_webui_action("Create a pawbot config and workspace now?", yes=yes)
    ...
    try:
        ...
        retry_command = f'pawbot webui --config "{config_path}"'
        _print_runtime_config_validation_error(
            exc,
            config_path=config_path,
