...
from pawbot.gateway.service import (
...
def create_gateway_app(
    *,
    ...
    gateway_app = typer.Typer(
        help="Start and manage the pawbot gateway.",
    ...
    def interactive_build_mode() -> BuildMode:
        # `pawbot gateway` is often launched by tests, supervisors, or service managers.
        # The higher-level `pawbot webui` command owns interactive first-run guidance.
        return "warn"
    ...
    def gateway(  # pyright: ignore[reportUnusedFunction]
        ...
        if background:
            ...
            if (
                ...
                console.print(
                    "[dim]Stop it first, then rerun `pawbot gateway --background` "
            ...
            if result.message == "gateway_already_running":
                if result.promoted:
                    ...
                    console.print(
                        ...
                        "use `pawbot gateway stop` to stop it.[/dim]"
                    )
                else:
                    console.print(
    ...
    def gateway_restart(  # pyright: ignore[reportUnusedFunction]
        ...
        if result.message == "gateway_not_running":
            ...
            console.print(
                "[dim]Start a persistent gateway with `pawbot gateway --background`.[/dim]"
            )
            print_status(result.status)
            raise typer.Exit(1)
    ...
    def gateway_install_service(  # pyright: ignore[reportUnusedFunction]
        ...
        verbose: bool = typer.Option(False, "--verbose", "-v", help="Verbose output"),
        config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
        name: str = typer.Option("pawbot-gateway", "--name", help="Service name"),
