...
def _status_command(config_path: Path) -> str:
    return f'pawbot status --config "{config_path}"'
...
def _print_model_setup_steps(config_path: Path) -> None:
    ...
    console.print(
        f"  WebUI: run [cyan]pawbot webui {escape(config_arg)}[/cyan], "
        "then open Settings → Models"
    )
    console.print(f"  CLI:   run [cyan]pawbot onboard --wizard {escape(config_arg)}[/cyan]")
...
def _load_inspection_config(
    ...
    from pawbot.config.errors import ConfigLoadError
    from pawbot.config.loader import get_config_path, load_config, set_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from pawbot.config.paths import get_cron_dir
...
def _provider_setup_error(config: Config) -> str | None:
    ...
    from pawbot.providers.factory import validate_provider_setup
