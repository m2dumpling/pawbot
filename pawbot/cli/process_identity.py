"""Give pawbot processes recognizable operating-system names."""
...
def _set_process_title(title: str) -> None:
    ...
    from setproctitle import (  # pyright: ignore[reportMissingImports]
        setproctitle,  # pyright: ignore[reportMissingImports, reportUnknownVariableType]
    )
...
def set_cli_process_identity(args: list[str]) -> None:
    """Name this CLI process after the pawbot role it is running."""
    if os.name == "nt":
        # Windows process managers use the console launcher's executable name,
        # which packaging already generates as ``pawbot.exe``.
        return
    role = args[0] if args and args[0] in _ROLES else None
    _set_process_title(f"pawbot-{role}" if role else "pawbot")
