...
if TYPE_CHECKING:
    from pawbot.gateway.runtime import GatewayRuntime
...
def _resolve_webui_config_path(config: str | None) -> Path:
    """Resolve the config path used by ``pawbot webui`` and bind loader state."""
...
def _validate_gateway_startup(config: Config) -> str | None:
    ...
    try:
        ...
        retry_command = f'pawbot gateway --config "{config_path}"'
        _print_runtime_config_validation_error(
            exc,
...
def _gateway_health_ready(host: str, port: int, *, timeout_s: float = 0.4) -> bool:
    """Return whether the pawbot gateway health endpoint responds OK."""
    import json
    import urllib.error
...
def _print_foreground_port_conflict(
    *,
    ...
    if gateway_running:
        console.print(
            "[yellow]A pawbot gateway is already running for this local instance.[/yellow]"
        )
    else:
        console.print(
            "[red]Error: pawbot cannot start because one of its local ports "
    ...
    if gateway_running:
        ...
        console.print("If this is an existing pawbot instance, use it or stop it first:")
    console.print("  [cyan]pawbot gateway status[/cyan]")
    console.print("  [cyan]pawbot gateway stop[/cyan]")
    console.print(
...
def _gateway_instance_command(
    ...
    *,
    ...
    parts = ["pawbot", "gateway", subcommand, "--config", str(config_path)]
    if workspace:
...
def _run_quick_start_for_webui(
    ...
    *,
    ...
    from pawbot.cli.onboard import run_quick_start_onboard
    ...
    try:
        ...
        console.print(
            "[yellow]Run `pawbot onboard --wizard` "
