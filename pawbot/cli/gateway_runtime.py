...
from pawbot.config.paths import is_default_workspace
from pawbot.config.schema import Config
from pawbot.gateway.runtime import GatewayInstance
from pawbot.security.network import is_loopback_host
from pawbot.session.keys import UNIFIED_SESSION_KEY, last_channel_from_metadata
from pawbot.utils.evaluator import evaluate_response, resolve_evaluator_prompt
from pawbot.utils.helpers import sync_workspace_templates
from pawbot.webui.build import BuildMode
from pawbot.webui.dev import WebUIDevError, WebUIDevServer
...
def _run_gateway(
    ...
    *,
    ...
    from pawbot.config.watcher import watch_config_file
    from pawbot.cron.bound_runner import run_bound_cron_job
    from pawbot.cron.service import CronJobSkippedError, CronService
    from pawbot.cron.session_turns import is_bound_cron_job
    from pawbot.cron.types import CronJob
    from pawbot.llm_usage import record_llm_call
    from pawbot.llm_usage.context import llm_usage_source
    from pawbot.providers.factory import (
        ProviderSnapshot,
    ...
    from pawbot.session.recovery import RecoveryCoordinator
    from pawbot.session.webui_turns import (
        WebuiTurnCoordinator,
        WebuiTurnRoutePolicy,
        build_webui_fallback_model_observer,
    )
    from pawbot.triggers.local_runner import run_local_trigger_queue
    ...
    console.print(f"{__logo__} Starting pawbot gateway version {__version__} on port {port}...")
    _prepare_webui_bundle_for_gateway(
        config,
    ...
    from pawbot.session.keys import session_key_for_channel
    ...
    # ADR-002/004: expose the Record & Replay blackbox + token governance as a
    # WebUI action dispatcher (the agent instance is captured by the getter).
    from pawbot.webui.blackbox_api import blackbox_action_factory

    _webui_blackbox_action = blackbox_action_factory(
        ...
        try:
            ...
            tasks = [
                asyncio.create_task(
                    ...
                    name="pawbot-config-watcher",
                ),
                asyncio.create_task(_run_agent(), name="pawbot-agent-loop"),
                asyncio.create_task(channels.start_all(), name="pawbot-channels"),
                asyncio.create_task(
