...
from pawbot.cli.process_identity import named_executable
from pawbot.cli.runtime_config import _model_display
...
from pawbot.webui.session_identity import is_webui_session_key, webui_chat_id
...
def launch_tui(
    ...
    *,
    ...
    try:
        env = os.environ.copy()
        env.pop("PAWBOT_TUI_WS_URL", None)
        env.pop("PAWBOT_TUI_API_TOKEN", None)
        env.update(
            {
                "PAWBOT_TUI_BOOTSTRAP_URL": f"{base_url}/webui/bootstrap",
                "PAWBOT_TUI_HEALTH_URL": _gateway_health_url(
                    config.gateway.host,
                    config.gateway.port,
                ),
                "PAWBOT_TUI_API_URL": base_url,
                "PAWBOT_TUI_MODEL": _model_display(config)[0],
                "PAWBOT_TUI_MODEL_PRESET": config.agents.defaults.model_preset or "default",
                "PAWBOT_TUI_WORKSPACE": str(tui_workspace),
                "PAWBOT_TUI_VERSION": __version__,
                "PAWBOT_TUI_ACCESS": (
                    "workspace access" if config.tools.restrict_to_workspace else "full access"
                ),
                "PAWBOT_TUI_THEME": theme,
                "PAWBOT_TUI_GATEWAY_STOP_COMMAND": _gateway_instance_command(
                    "stop",
        ...
        if bootstrap_secret:
            env["PAWBOT_TUI_BOOTSTRAP_SECRET"] = bootstrap_secret
        else:
            env.pop("PAWBOT_TUI_BOOTSTRAP_SECRET", None)
        if chat_id:
            env["PAWBOT_TUI_CHAT_ID"] = chat_id
        else:
            env.pop("PAWBOT_TUI_CHAT_ID", None)
        try:
...
def _resolve_tui_command() -> list[str]:
    override = os.environ.get("PAWBOT_TUI_BIN", "").strip()
    if override:
        executable = Path(override).expanduser().resolve(strict=False)
        if not executable.is_file():
            raise TuiUnavailableError(f"PAWBOT_TUI_BIN does not exist: {executable}")
    ...
    asset = f"pawbot-tui-{system}-{machine}{suffix}"
    ...
    if source_dir is not None:
        ...
        if not bun:
            raise TuiUnavailableError(
                ...
                "install Bun, then run `pawbot agent` again"
            )
    ...
    raise TuiUnavailableError(
        f"no native TUI archive is published for pawbot {__version__} on this platform; "
        ...
        "`pawbot agent --classic` if intentional"
    )
...
def _download_release_tui(asset: str) -> Path | None:
    ...
    if os.environ.get("PAWBOT_TUI_NO_DOWNLOAD") == "1":
        return None
...
def _read_release_asset(url: str, *, max_bytes: int) -> bytes:
    request = urllib.request.Request(url, headers={"User-Agent": f"pawbot/{__version__}"})
        ...
        def wait_for_ready(log_path: object) -> _GatewayHandle:
            ...
            if current.running:
                raise TuiUnavailableError(
                    ...
                    "Run `pawbot gateway status` and inspect logs at "
                    f"{log_path}; if it remains degraded, run `pawbot gateway restart`."
                )
            raise TuiUnavailableError(
        ...
        if status.running:
            if status.port not in {None, config.gateway.port}:
                raise TuiUnavailableError(
                    ...
                    "restart it or use `pawbot agent --classic`"
                )
            ...
            raise TuiUnavailableError(
                "the configured gateway port belongs to a different pawbot instance; "
                "stop that instance or use `pawbot agent --classic`"
            )
