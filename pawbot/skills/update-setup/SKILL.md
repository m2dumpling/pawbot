---
name: update-setup
description: "One-time setup wizard for the pawbot upgrade skill. Triggers: setup update, configure update, 设置更新, 初始化更新."
---

# Update Setup

Generate a personalized upgrade skill in Pawbot's agent workspace.
...
Use `exec` to run `pawbot --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v pawbot || true
python -m pip show pawbot-ai || true
pipx list | sed -n '/pawbot-ai/,+3p' || true
uv tool list | sed -n '/pawbot-ai/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how pawbot was
installed.
...
`question: "Where is your pawbot source checkout? Enter an absolute path or a path relative to this workspace:"`.

**Question 2 — Optional dependencies:**

```
question: "Which optional dependencies do you need? List names separated by spaces, or reply 'none'. Available: api, azure, bedrock, olostep. Channel dependencies are installed from their manifests when the gateway starts."
```
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "pawbot-ai[EXTRAS]" --force` |
| pipx | `pipx install --force "pawbot-ai[EXTRAS]"` |
| pip | `python -m pip install --upgrade "pawbot-ai[EXTRAS]"` |
...
---
name: update
description: "Upgrade pawbot to the latest version. Triggers: upgrade pawbot, update pawbot, 升级pawbot, 更新pawbot."
---
...
4. Use `exec` to verify: `pawbot --version`
5. Tell the user the new version. Say: "Run `/restart` to restart pawbot and apply the update. If `/restart` is unavailable in this channel, restart the pawbot process manually."
```

## Step 5: Confirm

Only after `write_file` succeeds, tell the user:
"Upgrade skill created. Say 'upgrade pawbot' when you want to update."
