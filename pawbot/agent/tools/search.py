...
from pawbot.agent.tools.filesystem import ListDirTool, _FsTool
...
def _match_glob(rel_path: str, name: str, pattern: str) -> bool:
    ...
    if "/" in normalized or normalized.startswith("**"):
        ...
        if "**" in pattern_parts:
            ...
            # ``PurePath.match`` has platform/version-specific globstar
            # corner cases.  Match complete path segments explicitly so
            # ``**`` consumes zero or more directories (``src/*.py`` and
            # ``src/**/*.py`` therefore have distinct, predictable meaning).
