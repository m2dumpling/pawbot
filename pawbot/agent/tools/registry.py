...
from pawbot.agent.tools.context import ContextAware, current_request_context
...
class ToolRegistry:
    ...
    def compactable_names(self) -> set[str]:
        """Names of tools that allow in-flight compaction of oversized results.

        ADR-003: eligibility comes from ``Tool.compactable`` so MCP and plugin
        tools participate without hard-coded allowlists.
        """
        return {
            tool.name
            for tool in self._tools.values()
            if getattr(tool, "compactable", True)
        }
