...
if typing.TYPE_CHECKING:
    ...
    from pawbot.runtime_context import RuntimeContextProvider
...
class Schema(ABC):
    ...
    Concrete types live in :mod:`pawbot.agent.tools.schema`; all implement
...
class Tool(ABC):
    ...
    def compactable(self) -> bool:
        """Whether in-flight oversized results may be compacted to a guidance
        message. Defaults to True: any tool can return more than fits the
        budget, and the compaction message instructs the model to retry
        narrowly instead of repeating the call (ADR-003)."""
        return True
