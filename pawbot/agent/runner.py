...
from pawbot.providers.conversation_state import (
...
from pawbot.utils.prompt_templates import render_template
...
@dataclass
class _TurnState:
    """Mutable per-run iteration state.

    ADR-005: the ReAct loop previously kept ~14 locals scattered across a
    450-line method. This groups them into one explicit object so the loop
    skeleton stays readable and each stage mutates a single well-defined
    state instead of reaching through closure variables.
    """
...
class AgentRunner:
    ...
    async def _run_core(
        ...
        ADR-005: the iteration skeleton is kept small — each stage lives in a
        dedicated method that mutates a single :class:`_TurnState`. Governance
        operates on a model copy (``messages_for_model``) so the persisted
        conversation is never touched; a governance failure must stop the run
    ...
    async def _run_tool_iteration(
        ...
        """Execute one model response's tool calls and append the results."""
        context.tool_calls = list(response.tool_calls)
        ...
        results, new_events = await execute_tool_calls(
            ...
            external_lookup_counts=state.external_lookup_counts,
    ...
    async def _run_terminal_iteration(
        self,
        spec: AgentRunSpec,
        hook: AgentHook,
        messages: list[dict[str, Any]],
        ...
        Returns ``"continue"`` to keep iterating or ``"done"`` to break out of
        the ReAct loop with ``state`` carrying the final outcome.
        """
        ...
        if response.finish_reason == "length":
            if len(state.length_recovery_parts) < _MAX_LENGTH_RECOVERIES:
                state.length_recovery_parts.append(
                ...
                logger.info(
                    ...
                    iteration,
                    spec.session_key or "default",
                    len(state.length_recovery_parts),
        ...
        # content deltas; emit this terminal segment into the same stream.
        if (
        ...
        if state.length_recovery_parts:
            state.final_content = (
    ...
    async def _run_max_iterations_fallback(
        self,
        spec: AgentRunSpec,
        hook: AgentHook,
        messages: list[dict[str, Any]],
        ...
        # Drain any remaining injections so they are appended to the history
        # instead of being re-published as independent inbound messages.
        ...
        if spec.finalize_on_max_iterations:
            terminal_content, state.usage = await self._try_finalize_after_max_iterations(
                spec,
                hook,
                messages,
                state.usage,
        ...
        if state.length_recovery_parts:
            ...
            state.final_content = (
                "".join(state.length_recovery_parts).rstrip() + terminal_tail
            ).strip()
            state.pending_stream_content = terminal_tail
        else:
            state.final_content = terminal_content
    ...
    async def _request_model(
        ...
        *,
        ...
        # (PAWBOT_STREAM_IDLE_TIMEOUT_S), but a stream that keeps producing
        # very slow deltas can still run forever. Use a more generous wall-clock
        # timeout for streaming while preserving PAWBOT_LLM_TIMEOUT_S=0 as an
    ...
    def _resolve_llm_timeout_s(spec: AgentRunSpec) -> float | None:
        ...
        if timeout_s is None:
            ...
            # Set PAWBOT_LLM_TIMEOUT_S=0 to disable.
            raw = os.environ.get("PAWBOT_LLM_TIMEOUT_S", "300").strip()
            try:
                timeout_s = float(raw)
