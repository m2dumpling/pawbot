...
from pawbot.utils.runtime import ensure_nonempty_tool_result
...
# Compaction eligibility now comes from the tool registry (Tool.compactable),
# so MCP and plugin tools participate without a hard-coded allowlist (ADR-003).
...
class ContextGovernor:
    ...
    def prepare_for_model(
        ...
        """Shape a model-copy of ``messages`` for one request.

        ADR-005: the pipeline is expressed as three explicit phases — repair
        malformed history, compact when the request would overflow, then repair
        again. The repair phase runs on both sides of the compaction phase
        because compaction can re-introduce orphaned tool results.
        """
        updated = self._repair_history(messages)
        updated = self._compact_context(config, updated, compacted_tool_call_ids)
        return self._repair_history(updated)

    def _repair_history(self, messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Normalize malformed persisted history into a legal message sequence."""
    ...
    def _compact_context(
        ...
        """Apply the three-tier budget: offload, in-flight compaction, snip."""
        updated = self.apply_tool_result_budget(config, messages)
        updated = self.compact_inflight_overflow(config, updated, compacted_tool_call_ids)
        return self.snip_history(config, updated)
    ...
    def _inflight_compaction_candidates(
        ...
        # ADR-003: compaction eligibility comes from the tool registry
        # (tool.compactable), so MCP and plugin tools participate without
        # hard-coded allowlists.
        compactable_tools = config.tools.compactable_names
