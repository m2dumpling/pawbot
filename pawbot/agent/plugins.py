...
def _load_manifest(plugin_root: Path) -> AgentPlugin | None:
    ...
    pawbot_value = extension_payload.get("dev.pawbot")
    pawbot = cast(dict[str, object], pawbot_value) if isinstance(pawbot_value, dict) else {}
    return AgentPlugin(
        name=name,
        root=plugin_root,
        description=_string(payload.get("description")),
        repository=_string(payload.get("repository")),
        display_name=_string(pawbot.get("displayName")) or name,
        category=_string(pawbot.get("category")) or "Plugin",
        accent_color=_accent_color(pawbot.get("accentColor")),
        logo=_plugin_logo(pawbot.get("logo"), plugin_root),
        permissions=_string_tuple(pawbot.get("permissions")),
    )
...
def _plugin_logo(value: object, plugin_root: Path) -> str | None:
    """Resolve pawbot's optional packaged logo extension."""
    if value is None:
        return None
