...
_PINNED_DNS_RESOLVER_LOCK = asyncio.Lock()
...
def httpx2_env_proxy_mounts() -> dict[str, httpx2.AsyncBaseTransport | None]:
    """Build HTTPX2 proxy mounts for the MCP v2 transport stack."""
    proxies = getproxies()
...
class PinnedDNSAsyncTransport(httpx.AsyncBaseTransport):
    ...
    _resolver_lock = _PINNED_DNS_RESOLVER_LOCK
...
class PinnedDNSAsyncTransport2(httpx2.AsyncBaseTransport):
    """HTTPX2 equivalent of :class:`PinnedDNSAsyncTransport` for MCP SDK v2.

    MCP v2 deliberately uses distinct HTTPX2 transport types. Keep its DNS
    validation and pinning identical to the rest of Pawbot's SSRF boundary.
    """
