...
from pawbot.providers.xai_oauth import (
    XAI_CLIENT_VERSION,
    XAIToken,
    get_xai_oauth_token,
)
...
def _build_headers(token: str, model: str) -> dict[str, str]:
    ...
    return {
        ...
        "x-grok-client-identifier": "pawbot",
        ...
        "User-Agent": f"pawbot/{__version__} (python)",
...
def _friendly_error(status_code: int, response_body: str | None = None) -> str:
    if status_code == 401:
        message = "xAI rejected the login. Sign in again with `pawbot provider login xai-grok`."
    elif status_code == 403:
        message = "This xAI account or subscription cannot access the Grok subscription endpoint."
    elif status_code == 426:
        message = "xAI requires a newer Grok client version. Update pawbot and try again."
