...
from pawbot.config.timezone import detect_system_timezone
from pawbot.config_base import Base
from pawbot.cron.types import CronSchedule

if TYPE_CHECKING:
    from pawbot.agent.tools.cli_apps import CliAppsToolConfig
    from pawbot.agent.tools.filesystem import FileToolsConfig
    from pawbot.agent.tools.image_generation import ImageGenerationToolConfig
    from pawbot.agent.tools.self import MyToolConfig
    from pawbot.agent.tools.shell import ExecToolConfig
    from pawbot.agent.tools.web import WebToolsConfig
...
class TranscriptionConfig(Base):
    ...
    enabled: bool = True
    provider: str | None = None  # Validated by pawbot.audio.transcription_registry.
    model: str | None = None
...
class ModelPresetConfig(Base):
    ...
    context_window_mode: Literal["auto", "manual"] = Field(
        default="auto",
        validation_alias=AliasChoices("contextWindowMode", "context_window_mode"),
        serialization_alias="contextWindowMode",
    )
...
class AgentDefaults(Base):
    ...
    bot_name: str = "pawbot"  # Display name shown in CLI prompts (e.g. "{name} is thinking...")
...
class ProviderConfig(Base):
    ...
    # pawbot/providers/openai_compat_provider.py. Kept duplicated here to
...
class ToolsConfig(Base):
    ...
    web: WebToolsConfig = Field(default_factory=lambda: _lazy_default("pawbot.agent.tools.web", "WebToolsConfig"))
    exec: ExecToolConfig = Field(default_factory=lambda: _lazy_default("pawbot.agent.tools.shell", "ExecToolConfig"))
    file: FileToolsConfig = Field(default_factory=lambda: _lazy_default("pawbot.agent.tools.filesystem", "FileToolsConfig"))
    cli_apps: CliAppsToolConfig = Field(default_factory=lambda: _lazy_default("pawbot.agent.tools.cli_apps", "CliAppsToolConfig"))
    my: MyToolConfig = Field(default_factory=lambda: _lazy_default("pawbot.agent.tools.self", "MyToolConfig"))
    image_generation: ImageGenerationToolConfig = Field(
        default_factory=lambda: _lazy_default("pawbot.agent.tools.image_generation", "ImageGenerationToolConfig"),
    )
...
class Config(BaseSettings):
    ...
    def resolve_default_preset(self) -> ModelPresetConfig:
        ...
        return ModelPresetConfig(
            ...
            context_window_mode=d.context_window_mode,
...
def _resolve_tool_config_refs() -> None:
    ...
    like ``from pawbot.config.schema import ExecToolConfig`` continue to work.
    """
    import sys
    ...
    from pawbot.agent.tools.self import MyToolConfig
    from pawbot.agent.tools.shell import ExecToolConfig
    from pawbot.agent.tools.web import WebFetchConfig, WebSearchConfig, WebToolsConfig
