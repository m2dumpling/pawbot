...
def get_config_path() -> Path:
    ...
    Delegates to ``pawbot.config.loader.get_config_path`` at call time so
    that importing this module never triggers a circular import during startup.
    """
    from pawbot.config.loader import get_config_path as _loader_get_config_path
...
def is_default_workspace(workspace: str | Path | None) -> bool:
    """Return whether a workspace resolves to pawbot's default workspace path."""
    current = Path(workspace).expanduser() if workspace is not None else Path.home() / ".pawbot" / "workspace"
...
def get_cli_history_path() -> Path:
    ...
    return Path.home() / ".pawbot" / "history" / "cli_history"


def get_legacy_sessions_dir() -> Path:
    """Return the legacy global session directory used for migration fallback."""
    return Path.home() / ".pawbot" / "sessions"
