...
def load_config(config_path: Path | None = None) -> Config:
    ...
    if not path.exists():
        try:
            ...
            raise ConfigLoadError(
                ...
                summary=(
                    ...
                    "Check that complex PAWBOT_* values use valid JSON."
                ),
            ) from exc
        ...
        _apply_model_capability_defaults(config)
...
def _apply_ssrf_whitelist(config: Config) -> None:
    ...
    from pawbot.security.network import configure_ssrf_whitelist

    configure_ssrf_whitelist(config.tools.ssrf_whitelist)


def _apply_model_capability_defaults(config: Config) -> None:
    """Upgrade legacy generic context budgets for known model IDs.

    Older configurations persisted 128K or 200K for every model. The provider
    registry now carries verified model limits, so loading a config should make
    a known model's actual capability visible to the runtime without requiring
    a manual edit to ``config.json``. Explicit smaller budgets remain intact;
    the migration only replaces the historical generic defaults or clamps a
    value above a known provider limit.
    """
    from pawbot.providers.registry import context_window_tokens_for

    defaults = config.agents.defaults
    if defaults.context_window_mode == "auto":
        defaults.context_window_tokens = context_window_tokens_for(
            defaults.provider,
            defaults.model,
    ...
    for preset in config.model_presets.values():
        if preset.context_window_mode == "auto":
            preset.context_window_tokens = context_window_tokens_for(
                preset.provider,
                preset.model,
