...
from pawbot.agent.skills import parse_skill_metadata, valid_skill_metadata
from pawbot.apps.protocol import app_manifest, compact_dict
from pawbot.config.paths import get_runtime_subdir
from pawbot.security.workspace_policy import is_path_within
...
PAWBOT_EXTENSION_REGISTRY_URL = "https://raw.githubusercontent.com/Re-bin/pawbot-extension/main/registry.json"
PAWBOT_EXTENSION_RAW_BASE = "https://raw.githubusercontent.com/Re-bin/pawbot-extension/main"
_CATALOG_SOURCES = (
    ...
    ("extensions", PAWBOT_EXTENSION_REGISTRY_URL, PAWBOT_EXTENSION_RAW_BASE, False),
)
...
class CliAppManager:
    ...
    def _manifest_source(self, app: dict[str, Any]) -> str:
        ...
        if source == "extensions":
            return "pawbot-extension"
        return f"cli-anything:{source}"

    def _trust_registry(self, app: dict[str, Any]) -> str:
        return "pawbot-extension" if str(app.get("_source") or "") == "extensions" else "cli-anything"
        ...
        *,
        ...
        remove = compact_dict({
            ...
            "verification": (
                ...
                else ["pawbot_state_absent", "managed_paths_absent"]
            ),
        })
        return app_manifest(
            app_id=name,
    ...
    def _fallback_skill(self, app: dict[str, Any]) -> str:
        ...
        description = (_catalog_description(app) or f"Use {display} from pawbot.")[:1024]
        return f"""---
...
Use this skill when the user asks pawbot to operate {display} through its installed CLI app.
...
"""
    ...
    def _with_pawbot_skill_note(self, content: str, app: dict[str, Any]) -> str:
        name = str(app.get("name") or "unknown")
        skill_name = _skill_name(name)
...
"""
    ...
    def install_skill(self, app: dict[str, Any]) -> Path:
        ...
        content = self._with_pawbot_skill_note(content, app)
    ...
    def _record_installed(self, app: dict[str, Any]) -> dict[str, Any]:
        from pawbot.agent.plugins import set_agent_plugin_enabled
    ...
    def uninstall(self, name: str) -> dict[str, Any]:
        ...
        if strategy == "bundled" and still_available:
            message = (
                f"Removed {app['display_name']} from pawbot. {entry_point} "
                "is still available because it is managed outside pawbot."
            )
