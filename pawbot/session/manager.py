...
from pawbot.config.paths import get_legacy_sessions_dir, get_runtime_subdir
from pawbot.providers.base import ProviderConversationState
...
from pawbot.utils.subagent_channel_display import scrub_subagent_announce_body
    ...
    def _read_workspace_id(cls, marker: Path) -> str:
        ...
        if not _WORKSPACE_ID_RE.fullmatch(value):
            raise RuntimeError(
                ...
                "restore its original 32-character identifier before starting pawbot"
            )
        return value
    ...
    def _find_workspace_namespace(cls, workspace: Path, root: Path) -> str | None:
        ...
        if len(matches) > 1:
            raise RuntimeError(
                ...
                "remove the stale namespace marker before starting pawbot"
            )
        ...
        try:
            with open(tmp_path, "x", encoding="utf-8") as f:
                metadata_line = {
                    ...
                    # Keep old pawbot releases able to read sessions written
