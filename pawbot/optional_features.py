"""Optional pawbot feature discovery and enablement."""
...
from pawbot.channels.registry import channel_default_enabled
...
def optional_dependency_groups_from_metadata() -> dict[str, list[str] | None]:
    ...
    try:
        extras = metadata("pawbot-ai").get_all("Provides-Extra") or []
        raw_requirements = requires("pawbot-ai") or []
    ...
    *,
    ...
    if name in _BUNDLED_FEATURE_ALIASES:
        payload = optional_features_payload(
            last_action={
                "ok": True,
                "message": f"Feature '{name}' is included with pawbot",
                "enabled": True,
            }
        )
