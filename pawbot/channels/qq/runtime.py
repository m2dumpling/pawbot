...
- Publish to Pawbot bus via BaseChannel._handle_message()
...
class QQConfig(Base):
    ...
    # Optional: directory to save inbound attachments. If empty, use pawbot get_media_dir("qq").
    media_dir: str = ""
...
class QQChannel(BaseChannel):
    ...
    def _init_media_root(self) -> Path:
        ...
        if self.config.media_dir:
            ...
            try:
                ...
                root = Path.home() / ".pawbot" / "media" / "qq"
        else:
            root = Path.home() / ".pawbot" / "media" / "qq"
    ...
    async def start(self) -> None:
        ...
        if not QQ_AVAILABLE:
            self.logger.error("SDK not installed. Run: pawbot plugins enable qq")
            return
