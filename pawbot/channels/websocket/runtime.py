"""WebSocket server channel: pawbot acts as a WebSocket server and serves connected clients."""
...
from pawbot.config.schema import Base
...
from pawbot.webui.gateway_services import GatewayServices
...
from pawbot.webui.inbound_commands import WebUICommandRouter
from pawbot.webui.metadata import (
    WEBSOCKET_TURN_OWNER_METADATA_KEY,
    WEBUI_TURN_METADATA_KEY,
)
from pawbot.webui.outbound_projection import WebUIOutboundProjector
from pawbot.webui.session_identity import is_valid_webui_chat_id
from pawbot.webui.transcript import WEBUI_TRANSCRIPT_INCOMPLETE_KEY
from pawbot.webui.websocket_logging import websockets_server_logger

if TYPE_CHECKING:
    from pawbot.bus.outbound_events import ProgressEvent, RecoveryStateEvent
...
class WebSocketConfig(Base):
    ...
    - ``token_issue_path``: If non-empty, **GET** (HTTP/1.1) to this path returns JSON
      ...
      pawbot and shares the asyncio loop, use a thread or async HTTP client for GET—do not call
      blocking ``urllib`` or synchronous ``httpx`` from inside a coroutine.
    - ``token_issue_secret``: If non-empty, token requests must send ``Authorization: Bearer <secret>`` or
      ``X-Pawbot-Auth: <secret>``.
...
class WebSocketChannel(BaseChannel):
    ...
    async def start(self) -> None:
        from pawbot.utils.logging_bridge import redirect_lib_logging
