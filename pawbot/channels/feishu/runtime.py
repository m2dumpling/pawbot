...
from pawbot.channels.contracts import ChannelInstanceSpec
from pawbot.channels.feishu.config import FeishuConfig, feishu_default_config
from pawbot.channels.feishu.instances import (
    DEFAULT_INSTANCE_ID,
...
from pawbot.channels.feishu.websocket import get_feishu_ws_runner
from pawbot.command.router import normalize_command_text
from pawbot.config.paths import get_media_dir
from pawbot.pairing import clear_channel
...
def save_registration_result(
    ...
    *,
    ...
    default_name = (
        ...
        else f"pawbot {effective_instance_id}"
    )
...
class FeishuChannel(BaseChannel):
    ...
    async def login(self, force: bool = False) -> bool:
        ...
        _LOGIN_CONSOLE.print("Authorize with the mobile app. pawbot will save the new bot credentials.\n")

        result = qr_register(initial_domain=self.config.domain or "feishu")
        if not result:
            _LOGIN_CONSOLE.print(
                "[yellow]Login was not completed.[/yellow] "
                "Run 'pawbot channels login feishu --force' to retry."
            )
            return False
    ...
    async def start(self) -> None:
        ...
        if not FEISHU_AVAILABLE:
            self.logger.error("SDK not installed. Run: pawbot plugins enable feishu")
            return

        if not self.config.app_id or not self.config.app_secret:
            self.logger.error(
                "app_id and app_secret not configured. "
                "Run 'pawbot channels login feishu' to set up via QR code."
            )
            return
