...
import { enablePawbotFeature } from "@/lib/api";
import type {
...
function FeishuInstanceAction({
  ...
  const reconnect = async () => {
    ...
    try {
      onFeaturesUpdate(
        await enablePawbotFeature(client, "feishu", { instanceId: instance.id }),
      );
    } catch (err) {
...
function defaultFeishuInstance(feature: PawbotFeatureInfo): PawbotChannelInstanceInfo {
  return {
    id: "default",
    name: "pawbot",
...
function instanceDisplayName(instance: PawbotChannelInstanceInfo): string {
