...
export function WeixinPanel({
  ...
  const saveSettings = useCallback(async (
    ...
    try {
      ...
      if (payload.pawbot_features) context.onFeaturesUpdate(payload.pawbot_features);
    } catch (err) {
        ...
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-[13px] leading-5 text-muted-foreground">
              {channelTx("description", "Use pawbot from WeChat conversations.")}
            </p>
            {missingSupport && feature.install_supported ? (
              ...
              >
                ...
                {tx("settings.pawbotFeatures.installSupport", "Install support")}
              </Button>
            ) : null}
          </div>
        </div>
