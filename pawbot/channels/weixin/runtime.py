...
from pawbot.config.paths import get_media_dir, get_runtime_subdir
...
BASE_INFO: dict[str, str] = {
    ...
    "bot_agent": f"pawbot/{__version__} (python)",
}
...
class WeixinConfig(Base):
    ...
    state_dir: str = ""  # Default: ~/.pawbot/weixin/
...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                self.logger.error("login failed. Run 'pawbot channels login weixin' to authenticate.")
                self._running = False
                return
    ...
    def _assert_session_active(self) -> None:
        if self._auth_required:
            raise WeixinAuthError(
                ...
                errmsg="bot token is stale; run 'pawbot channels login weixin --force'",
            )
        ...
        try:
            ...
            raise WeixinAuthError(
                ...
                errmsg=(
                    ...
                    "run 'pawbot channels login weixin --force'"
                ),
            ) from None
    ...
    async def _send_text(
        ...
        *,
        ...
        client_id = client_id or f"pawbot-{uuid.uuid4().hex[:12]}"
...
def _encrypt_aes_ecb(data: bytes, aes_key_b64: str) -> bytes:
    ...
    try:
        ...
        logger.warning(
            "Cannot encrypt media. Run `pawbot plugins enable weixin` "
...
def _decrypt_aes_ecb(data: bytes, aes_key_b64: str) -> bytes:
    ...
    if decrypted is None:
        try:
            ...
            logger.warning(
                "Cannot decrypt media. Run `pawbot plugins enable weixin` "
                "to install WeChat support."
            )
            return data
