...
try:
    import pawbot.channels.dingtalk.runtime as dingtalk_module
...
from pawbot.channels.dingtalk.runtime import (
    DingTalkChannel,
    DingTalkConfig,
    PawbotDingTalkHandler,
)
...
async def test_handler_uses_voice_recognition_text_when_text_is_empty(monkeypatch) -> None:
    ...
    handler = PawbotDingTalkHandler(channel)
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/pawbot_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/pawbot_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "pawbot.config.paths.get_media_dir",
