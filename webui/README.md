...
This directory contains the React/TypeScript source for the pawbot WebUI. If
you only want to use the bundled browser UI, follow the gateway commands in the
root [`README.md`](../README.md). You do not need Node.js, Bun, Vite, or
...
|---|---|---|
| Use the bundled browser UI | [`README.md`](../README.md#first-conversation) | `http://127.0.0.1:8765` |
| Use the WebUI from another device | [`README.md`](../README.md#first-conversation) | `http://<your-ip>:8765` |
| Change WebUI source code | [Develop the WebUI (Vite HMR)](#develop-the-webui-vite-hmr) | `http://127.0.0.1:5173` |
| Debug setup failures | [`README.md`](../README.md#security-and-data-boundary) | Configuration and security boundaries |
...
pawbot/web/dist/      build output served by the gateway
```

## Develop the WebUI (Vite HMR)

### 1. Install pawbot from source
...
```bash
PAWBOT_API_URL=http://127.0.0.1:9000 bun run dev
```
...
```bash
cd webui
bun run build          # writes to ../pawbot/web/dist
```
