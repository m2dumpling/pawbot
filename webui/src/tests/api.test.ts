...
describe("webui API helpers", () => {
  ...
  it("reports HTML API fallbacks as gateway mismatch errors", async () => {
    ...
    await expect(fetchApiService("tok")).rejects.toMatchObject({
      status: 200,
      message: "Gateway returned WebUI HTML instead of JSON. Restart pawbot gateway and try again.",
    });
  });
  ...
  it("reads and toggles pawbot optional features", async () => {
    vi.mocked(fetch).mockResolvedValueOnce({
      ok: true,
      json: async () => ({
        features: [],
    ...
    await expect(fetchPawbotFeatures("tok")).resolves.toMatchObject({ features: [] });
    expect(fetch).toHaveBeenCalledWith(
    ...
    await enablePawbotFeature(mutationTransport, "matrix");
    expect(requestMutation).toHaveBeenLastCalledWith(
    ...
    await disablePawbotFeature(mutationTransport, "matrix");
    expect(requestMutation).toHaveBeenLastCalledWith(
  ...
  it("reads and writes persisted sidebar state", async () => {
    const state = {
      ...
      project_name_overrides: { "/Users/me/pawbot": "Core" },
      tags_by_key: {},
      collapsed_groups: {},
      view: {
