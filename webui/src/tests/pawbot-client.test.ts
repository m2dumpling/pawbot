...
/**
 * Minimal fake WebSocket implementing the subset PawbotClient touches.
...
describe("PawbotClient", () => {
  ...
  it("can swap the socket factory when the runtime URL changes", () => {
    ...
    expect(hostFactory).toHaveBeenCalledWith("pawbot-host://engine/");
    expect(lastSocket().url).toBe("host:pawbot-host://engine/");
  });
  ...
  it("uses the host socket bridge for native host URLs", async () => {
    ...
    expect(openSocket).toHaveBeenCalledWith("pawbot-host://engine/");
