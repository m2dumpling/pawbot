...
const LOCALIZED_SETTINGS_COPY_KEYS = [
  ...
  "settings.pawbotFeatures.disable",
  "settings.pawbotFeatures.ready",
  "settings.pawbotFeatures.missingDependency",
  "settings.pawbotFeatures.installConfirmTitle",
  "settings.pawbotFeatures.installConfirmDescription",
  "settings.pawbotFeatures.installConfirmAction",
  "settings.pawbotFeatures.channelDisabled",
  "settings.pawbotFeatures.notEnabled",
...
const PREBOOT_SCRIPT = INDEX_HTML.match(
  /<script>\s*(\(function \(\) \{\s*var localeKey = "pawbot\.locale";[\s\S]*?\}\)\(\);)\s*<\/script>/,
)?.[1];
const BOOT_COPY_MARKUP = '<span data-boot-copy>Loading pawbot…</span>';
...
describe("webui i18n", () => {
  ...
  it("switches UI copy and document locale through the language switcher", async () => {
    ...
    expect(
      screen.getByPlaceholderText("Describe the task you'd like done…"),
    ).toBeInTheDocument();
    ...
    expect(localStorage.getItem("pawbot.locale")).toBe("zh-CN");
  ...
  it("keeps Simplified Chinese settings overview copy localized", () => {
    ...
    expect(settings.skills.marketplaceProviderFilter).toBe("指令包来源");
    expect(settings.skills.marketplaceProviderAll).toBe("全部");
    expect(settings.skills.marketplaceSearchPlaceholder).toBe("搜索指令包");
    expect(settings.skills.marketplaceTrendingTitle).toBe("各市场热门指令包");
  });
