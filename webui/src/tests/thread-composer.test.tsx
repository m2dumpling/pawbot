...
afterEach(() => {
  ...
  vi.unstubAllGlobals();
  vi.useRealTimers();
  Reflect.deleteProperty(window, "pawbotHost");
...
describe("reasoning effort selector", () => {
  it("shows model-specific effort levels and reports selection", async () => {
    ...
    render(
      <ThreadComposer
        ...
        modelPresets={MODEL_PRESETS.map((preset) => ({
          ...preset,
          reasoningEffort: preset.name === "dflash" ? "high" : null,
          reasoningEffortValues: preset.name === "dflash"
            ? ["", "low", "high", "max"]
            : undefined,
        }))}
    ...
    fireEvent.click(screen.getByRole("button", { name: "dflash" }));
    const picker = screen.getByRole("dialog", { name: "Switch model for this chat" });

    expect(within(picker).getByText("Thinking effort")).toBeInTheDocument();
    const maxOption = within(picker).getByRole("option", { name: "max" });
    const highOption = within(picker).getByRole("option", { name: "high" });
  ...
  it("hides the effort section when the model exposes no levels", () => {
    render(
      <ThreadComposer
        onSend={vi.fn()}
        modelLabel="kimi"
    ...
    const picker = screen.getByRole("dialog", { name: "Switch model for this chat" });
    expect(within(picker).queryByText("Thinking effort")).not.toBeInTheDocument();
  });
});
...
describe("ThreadComposer", () => {
  ...
  it("renders a readonly hero model composer when provided", () => {
    ...
    expect(input.className).toContain("min-h-[92px]");
    expect(input.className).toContain("text-[16px]");
    expect(input.className).toContain("pt-[30px]");
    fireEvent.change(input, { target: { value: "1" } });
    expect(input.className).toContain("pt-[30px]");
    expect(input.parentElement?.parentElement?.className).toContain("max-w-[64rem]");
  });
  ...
  it("keeps the thread composer compact while matching the hero style", () => {
    ...
    expect(modelPill).toHaveClass("font-medium", "text-foreground/75");
    ...
    expect(input.className).toContain("text-[16px]");
    expect(input.parentElement?.parentElement?.className).toContain("max-w-[56rem]");
    expect(input.parentElement?.parentElement?.className).toContain("rounded-[1.35rem]");
    expect(input.parentElement?.parentElement?.className).toContain("shadow-");
    expect(screen.getByRole("button", { name: "Attach files" }).className).toContain("bg-background");
  ...
  it("keeps project selection as a compact composer dropdown", async () => {
    ...
    const onWorkspaceScopeChange = vi.fn();
    const defaultScope = {
      project_path: "/Users/test/.pawbot/workspace",
  ...
  it.each([
    ["Windows", "D:\\Users\\test\\.pawbot\\workspace", "D:\\path\\to\\project"],
    ["macOS", "/Users/test/.pawbot/workspace", "/Users/name/project"],
    ["Linux", "/home/test/.pawbot/workspace", "/home/name/project"],
  ...
  it("uses the native folder picker for project selection on native host", async () => {
    ...
    Object.defineProperty(window, "pawbotHost", {
      configurable: true,
      value: {
  ...
  it("prioritizes stop as an immediate slash action while streaming", () => {
    ...
    expect(input).toHaveValue("");
    expect(window.localStorage.getItem("pawbot.webui.slashCommandRecents")).toBeNull();
  });

  it("orders recent slash commands first for the blank slash menu", () => {
    window.localStorage.setItem("pawbot.webui.slashCommandRecents", JSON.stringify(["/history"]));
    render(
      <ThreadComposer
        onSend={vi.fn()}
        placeholder="Type your message..."
  ...
  it("attaches persisted sessions only through the shared mention palette", () => {
    ...
    expect(screen.getByRole("group", { name: "Pawbot conversations" })).toBeInTheDocument();
  ...
  it("keeps temporary chat guidance in memory only", async () => {
    ...
    expect(
      window.localStorage.getItem(
        "pawbot.webui.composerQueuedGuidance.v1:temporary-private",
      ),
    ).toBeNull();
