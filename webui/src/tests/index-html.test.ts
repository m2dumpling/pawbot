...
describe("index.html", () => {
  ...
  it("provides light and dark PWA chrome colors", () => {
    ...
    expect(themeColor?.dataset.themeColorDark).toBe("#151517");
    expect(lightBodyBackground).toBe("#ffffff");
    expect(darkBodyBackground).toBe("#151517");
    expect(manifest.background_color).toBe("#ffffff");
    expect(manifest.theme_color).toBe("#ffffff");
    expect(manifest.color_scheme_dark?.background_color).toBe("#151517");
    expect(manifest.color_scheme_dark?.theme_color).toBe("#151517");
  });
});
