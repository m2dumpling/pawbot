...
describe("ChatList", () => {
  ...
  it("marks a conversation that needs recovery attention with a warning indicator", () => {
    ...
    expect(screen.getByRole("img", { name: "This task needs your attention" }))
      .toBeInTheDocument();
  });
  ...
  it("shows the running indicator while a recovery continuation is active", () => {
    ...
    expect(screen.queryByRole("img", { name: "This task needs your attention" }))
  ...
  it("shows every tab's pane membership in a sidebar tab group", async () => {
    ...
    fireEvent.pointerDown(screen.getByRole("button", {
      name: "Task actions for Root topic",
    }), { button: 0, ctrlKey: false });
  ...
  it("selects a contiguous session range with Shift-click", async () => {
    ...
    fireEvent.pointerDown(screen.getByRole("button", {
      name: "Task actions for First topic",
    }), { button: 0, ctrlKey: false });
    fireEvent.click(await screen.findByRole("menuitem", { name: "Select" }));
  ...
  it("shows temporary chats separately and lets the user reopen or close them", async () => {
    ...
    const section = screen.getByRole("region", { name: "Quick sessions" });
  ...
  it("groups WebUI chats by workspace project while preserving in-project sorting and activity", () => {
    const sessions = [
      ...
      session({
        ...
        workspaceScope: {
          project_path: "/Users/me/pawbot-bench",
          project_name: "pawbot-bench",
          access_mode: "full",
        },
      }),
    ];
    ...
    const pawbotSection = screen.getByRole("region", { name: "pawbot" });
    const pawbotText = pawbotSection.textContent ?? "";
    const projectSurface = pawbotSection.querySelector(
      "[data-sidebar-project-surface]",
    );

    expect(screen.getByRole("region", { name: "pawbot-bench" })).toBeInTheDocument();
    expect(projectSurface).toHaveClass(
      "rounded-es-[16px]",
      "border-s-2",
      "border-sidebar-foreground/10",
    );
    expect(within(pawbotSection).getByText("Alpha task")).toBeInTheDocument();
    expect(within(pawbotSection).getByText("Zeta task")).toBeInTheDocument();
    expect(pawbotText.indexOf("Alpha task")).toBeLessThan(pawbotText.indexOf("Zeta task"));
    expect(within(pawbotSection).getByLabelText("Agent running")).toBeInTheDocument();
  ...
  it("keeps default workspace topics in the Topics section instead of a project folder", () => {
    const sessions = [
      session({
        ...
        updatedAt: "2026-05-21T10:00:00Z",
        workspaceScope: {
          project_path: "/Users/me/.pawbot/workspace",
    ...
    render(
      <ChatList
        ...
        onRequestRename={vi.fn()}
        onToggleArchive={vi.fn()}
        defaultWorkspacePath="/Users/me/.pawbot/workspace"
        showTimestamps
      />,
    );

    expect(screen.getByText("Projects")).toBeInTheDocument();
    expect(screen.getByRole("region", { name: "pawbot" })).toBeInTheDocument();
    expect(screen.queryByRole("region", { name: "workspace" })).not.toBeInTheDocument();

    const chatsSection = screen.getByRole("region", { name: "Tasks" });
  ...
  it("can collapse a project group and keeps project rename separate from chat titles", async () => {
    ...
    render(
      <ChatList
        ...
        projectNameOverrides={{ "/Users/me/pawbot": "Photos" }}
        collapsedGroups={{ "project:/Users/me/pawbot": true }}
      />,
    );
    ...
    expect(onToggleGroup).toHaveBeenCalledWith("project:/Users/me/pawbot");
    ...
    fireEvent.click(await screen.findByRole("menuitem", { name: "Start a task" }));
    expect(onNewChatInProject).toHaveBeenCalledWith("/Users/me/pawbot", "Photos");
    expect(onToggleGroup).toHaveBeenCalledTimes(1);

    fireEvent.contextMenu(projectButton);
    fireEvent.click(await screen.findByRole("menuitem", { name: "Rename" }));

    expect(onRequestRenameProject).toHaveBeenCalledWith("/Users/me/pawbot", "Photos");
  });
  ...
  it("animates project disclosure and surrounding layout like tab groups", () => {
    ...
    expect(screen.getByRole("button", { name: "Task actions for Alpha project" })
  ...
  it("folds long default workspace chats and can show all", () => {
    ...
    fireEvent.click(within(chatsSection).getByRole("button", { name: "2 hidden tasks" }));
  ...
  it("sorts Topics section among project groups by recency, not always last", () => {
    ...
    const chatsIdx = regionNames.findIndex((n) => n?.includes("Tasks"));
  ...
  it("keeps one Projects heading when Topics sorts between project groups", () => {
    ...
    expect(regionNames).toEqual(["project-a", "Tasks", "project-b"]);
  ...
  it("keeps Topics last when its latest conversation is older than all projects", () => {
    ...
    expect(regionNames).toEqual(["project-a", "project-b", "Tasks"]);
    expect(screen.getAllByText("Projects")).toHaveLength(1);
  });
});
