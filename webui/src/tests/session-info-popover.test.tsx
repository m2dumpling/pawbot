...
describe("SessionInfoPopover", () => {
  ...
  it("shows the actual message received by a local trigger", async () => {
        ...
        automationsResponse([
          {
            ...
            payload: {
              ...
              command: 'pawbot trigger trg_123 "message"',
            },
            state: { pending: false },
          },
        ]),
    ...
    expect(screen.queryByText('pawbot trigger trg_123 "message"')).not.toBeInTheDocument();
  });
  ...
  it("refreshes while open so completed one-shot automations disappear", async () => {
    ...
    expect(screen.getByText("No scheduled tasks in this session yet.")).toBeInTheDocument();
  }, 8000);
