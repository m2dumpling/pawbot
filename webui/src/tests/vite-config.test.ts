...
describe("gzipWebuiAssets", () => {
  it("compresses finalized files after Rollup writes the bundle", () => {
    ...
    const outputDir = mkdtempSync(path.join(tmpdir(), "pawbot-vite-gzip-"));
    try {
...
describe("webuiManualChunk", () => {
  it("splits only root application surfaces to avoid chunk cycles", () => {
    expect(webuiManualChunk("/repo/webui/src/components/Sidebar.tsx")).toBe("sidebar-ui");
    expect(webuiManualChunk("/repo/webui/src/components/thread/ThreadShell.tsx")).toBe(
      "thread-ui",
    );
    expect(webuiManualChunk("/repo/webui/src/components/workbench/PaneWorkbench.tsx")).toBe(
      "workbench-ui",
    );
    expect(webuiManualChunk("/repo/webui/src/components/thread/ThreadHeader.tsx")).toBeUndefined();
  });
