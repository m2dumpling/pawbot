...
describe("Settings overview and appearance", () => {
  ...
  it("persists the file edit display local preference", async () => {
    ...
    await waitFor(() => {
      const saved = JSON.parse(localStorage.getItem("pawbot-webui.settings-preferences") || "{}");
  ...
  it("shows a compact empty state before any token activity exists", () => {
    const payload: SettingsPayload = {
      ...settingsPayload(),
      usage: {
        days: [],
    ...
    expect(screen.getByText("Token activity will appear after new model replies.")).toBeInTheDocument();
