...
describe("Settings channels", () => {
  ...
  it("starts Feishu connect in WebUI instead of showing a CLI command", async () => {
    ...
    expect(screen.queryByText("pawbot channels login feishu")).not.toBeInTheDocument();
  ...
  it("enables configured Feishu assistant without starting a new connect flow", async () => {
    ...
    fireEvent.click(await screen.findByRole("button", { name: "pawbot" }));
    fireEvent.click(await screen.findByRole("switch", { name: "pawbot assistant" }));
    ...
    expect(screen.getByRole("switch", { name: "pawbot assistant" })).toHaveAttribute(
      "aria-checked",
      "true",
    );
  });
  ...
  it("shows a single Feishu assistant without a duplicate assistant list", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: RequestInfo | URL) => {
        ...
        if (url === "/api/settings/cli-apps") return jsonResponse({ apps: [], installed_count: 0 });
        if (url === "/api/settings/mcp-presets") return jsonResponse({ presets: [], installed_count: 0 });
        if (url === "/api/settings/pawbot-features") return jsonResponse(feishuPayload);
        return { ok: false, status: 404, json: async () => ({}) } as Response;
      }),
    );
  ...
  it("uses choices for channel enum and boolean fields", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "pawbot" }));
    fireEvent.click(screen.getByText("Advanced"));
