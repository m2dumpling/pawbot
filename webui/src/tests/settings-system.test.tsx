...
describe("Settings system domains", () => {
  ...
  it("offers a way out of an empty automations filter", async () => {
    ...
    expect(await screen.findByText("No scheduled tasks match this view.")).toBeInTheDocument();
  ...
  it("starts the managed API server from System", async () => {
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (url === "/api/settings/pawbot-features") {
  ...
  it("shows a visible uninstall button for installed CLI apps and calls uninstall", async () => {
    ...
    expect(screen.queryByRole("heading", { name: "Toolbox" })).not.toBeInTheDocument();
  ...
  it("keeps runtime dependencies out of Apps and explains chat mentions", async () => {
    ...
    expect(await screen.findByText("AnyGen")).toBeInTheDocument();
    expect(
      screen.queryByText("Add tools to pawbot, then @ them in chat."),
    ).not.toBeInTheDocument();
  ...
  it("shows pawbot optional features and enables one", async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      const url = String(input);
      if (url === "/api/settings") return jsonResponse(settingsPayload());
      if (url === "/api/settings/cli-apps") return jsonResponse({ apps: [], installed_count: 0 });
    ...
    expect(screen.getAllByText("Use pawbot from Matrix rooms.")).toHaveLength(2);
    expect(screen.queryByText(/Enabling Pawbot features may install Python packages/)).not.toBeInTheDocument();
    ...
    expect(screen.getByText("pawbot will add what Matrix needs, then turn it on. Continue?")).toBeInTheDocument();
    ...
    expect(screen.queryByText("Restart pawbot to apply updated channel support.")).not.toBeInTheDocument();
