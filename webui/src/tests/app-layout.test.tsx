...
const HERO_GREETING_PATTERN =
  /What should we get done today\?|Give me a task — I'll run with it\.|What are you building\?|What problem should we take on\?/;
...
vi.mock("@/lib/pawbot-client", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/lib/pawbot-client")>();
  class MockClient {
  ...
  return { ...actual, PawbotClient: MockClient };
});
...
describe("App layout", () => {
  beforeEach(async () => {
    ...
    localStorage.removeItem("pawbot-webui.sidebar");
    localStorage.removeItem("pawbot-webui.sidebar.completed-runs.v1");
    localStorage.removeItem("pawbot-webui.sidebar.session-updates.v1");
    localStorage.removeItem("pawbot-webui.collapsed-pane-groups.v1");
    localStorage.removeItem("pawbot-webui.restartStartedAt");
    localStorage.removeItem("pawbot-webui.restartRoute");
  ...
  it("uses one main landmark and a page heading in desktop settings", async () => {
    ...
    fireEvent.click(within(sidebar).getByRole("button", { name: "Preferences" }));
  ...
  it("places Scheduled tasks after Prompt packs in the main sidebar", async () => {
    render(<App />);

    await waitFor(() => expect(connectSpy).toHaveBeenCalled());
    const sidebar = screen.getByRole("navigation", { name: "Sidebar navigation" });
    const appsButton = within(sidebar).getByRole("button", { name: "Toolbox" });
    const skillsButton = within(sidebar).getByRole("button", { name: "Prompt packs" });
    const automationsButton = within(sidebar).getByRole("button", { name: "Scheduled tasks" });
  ...
  it("highlights the blank new-task destination immediately", async () => {
    render(<App />);

    await waitFor(() => expect(connectSpy).toHaveBeenCalled());
    const sidebar = screen.getByRole("navigation", { name: "Sidebar navigation" });
    const newTaskButton = within(sidebar).getByRole("button", { name: "Start a task" });

    expect(newTaskButton).toHaveAttribute("aria-current", "page");
    expect(newTaskButton).not.toHaveClass("bg-sidebar-accent");
    expect(newTaskButton).toHaveClass("bg-primary/[0.06]");
    expect(newTaskButton).toHaveClass("border-primary/25");
  });
  ...
  it("creates a new temporary chat from the hero each time", async () => {
    ...
    await waitFor(() => expect(connectSpy).toHaveBeenCalled());
    const sidebar = screen.getByRole("navigation", { name: "Sidebar navigation" });
    expect(within(sidebar).queryByRole("button", { name: "Quick session" })).not.toBeInTheDocument();
    const firstToggle = screen.getByRole("button", { name: "Quick session" });
    ...
    expect(screen.queryByRole("button", { name: "Quick session" })).not.toBeInTheDocument();
    expect(createChatSpy).not.toHaveBeenCalled();

    fireEvent.click(within(sidebar).getByRole("button", { name: "Start a task" }));
    expect(discardTemporaryChatSpy).not.toHaveBeenCalled();
    const secondToggle = screen.getByRole("button", { name: "Quick session" });
    ...
    expect(within(sidebar).getByText("Quick sessions")).toBeInTheDocument();
    ...
    await waitFor(() => expect(document.title).toBe("first private message · pawbot"));
    expect(screen.queryByRole("button", { name: "Quick session" })).not.toBeInTheDocument();

    fireEvent.click(within(sidebar).getByRole("button", {
      name: "Close quick session: first private message",
    }));
  ...
  it("shows the temporary-chat control only on the new-topic hero", async () => {
    ...
    expect(temporaryTooltip).toHaveTextContent("Quick session");
    ...
    fireEvent.click(within(sidebar).getByRole("button", { name: "Start a task" }));
    const temporaryToggle = screen.getByRole("button", { name: "Quick session" });
    expect(temporaryToggle).toHaveClass("h-8", "w-8", "rounded-[0.65rem]");
    expect(within(temporaryToggle).queryByText("Quick session")).not.toBeInTheDocument();
    ...
    expect(screen.queryByRole("button", { name: "Clear quick session" })).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Quick session" })).not.toBeInTheDocument();
  });
  ...
  it("restores the Settings route after a restart fallback hash", async () => {
    localStorage.setItem("pawbot-webui.restartStartedAt", String(Date.now()));
    localStorage.setItem("pawbot-webui.restartRoute", "#/settings?section=channels");
    window.history.replaceState(null, "", "/#/new");
    mockFetchRoutes({
      "/api/settings": baseSettingsPayload(),
      "/api/settings/pawbot-features": {
        features: [{
          name: "websocket",
    ...
    await waitFor(() => expect(connectSpy).toHaveBeenCalled());
    expect(
      await screen.findByRole("navigation", { name: "Preference sections" }),
    ).toBeInTheDocument();
  ...
  it("opens Skills from the main sidebar", async () => {
    ...
    expect(screen.getByRole("textbox", { name: "Search installed prompt packs" })).toBeInTheDocument();
    ...
    expect(screen.queryByRole("navigation", { name: "Preference sections" })).not.toBeInTheDocument();
    expect(within(sidebar).getByRole("button", { name: "Prompt packs" })).toHaveAttribute(
      "aria-current",
      "page",
    );
    expect(document.title).toBe("Prompt packs · pawbot");
    ...
    fireEvent.click(within(sidebar).getByRole("button", { name: "Prompt packs" }));
    expect(await screen.findByRole("heading", { name: "Prompt packs" })).toBeInTheDocument();
    ...
    fireEvent.click(screen.getByText("Prompt pack instructions"));
  ...
  it("deletes a custom skill from its detail sheet", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Delete prompt pack" }));
    ...
    expect(screen.getByText("No matching prompt packs.")).toBeInTheDocument();
  });
  ...
  it("discovers and installs a skill from skills.sh", async () => {
    ...
    fireEvent.change(screen.getByRole("textbox", { name: "Search prompt packs" }), {
      target: { value: "React" },
    });
    ...
    fireEvent.click(screen.getByRole("button", { name: "Install prompt pack" }));
  ...
  it("opens Automations from the main sidebar", async () => {
    ...
    expect(within(automationsMain as HTMLElement).queryByText("Preferences")).not.toBeInTheDocument();
    ...
    expect(within(sidebar).getByRole("button", { name: "Scheduled tasks" })).toHaveAttribute(
      "aria-current",
      "page",
    );
    expect(document.title).toBe("Scheduled tasks · pawbot");
  ...
  it("edits a past one-time automation without resubmitting its old schedule", async () => {
    ...
    await waitFor(() => expect(connectSpy).toHaveBeenCalled());
    const sidebar = screen.getByRole("navigation", { name: "Sidebar navigation" });
    fireEvent.click(within(sidebar).getByRole("button", { name: "Scheduled tasks" }));
  ...
  it("localizes the Automations surface", async () => {
    ...
    fireEvent.click(within(sidebar).getByRole("button", { name: "定时任务" }));
    ...
    expect(document.title).toBe("定时任务 · pawbot");
  });
  ...
  it("switches to the next session when deleting the active chat", async () => {
    ...
    fireEvent.pointerDown(screen.getByLabelText("Task actions for First chat"), {
      button: 0,
    });
  ...
  it("deletes multiple selected topics through one confirmation", async () => {
    ...
    expect(await screen.findByText("Delete 2 tasks?")).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Delete" }));
  ...
  it("shows localized bound automations in the first delete confirmation", async () => {
    ...
    fireEvent.pointerDown(screen.getByLabelText(/First chat.*任务操作/), {
      button: 0,
    });
    ...
    expect(getSessionAutomationsSpy).toHaveBeenCalledWith("websocket:chat-a");
    expect(
      screen.getByText("这个任务有关联的定时任务。删除任务也会删除这些定时任务。"),
    ).toBeInTheDocument();
    expect(
  ...
  it("keeps the mobile session action menu inside the sidebar sheet", async () => {
    ...
    fireEvent.pointerDown(
      within(mobileSidebar).getByLabelText("Task actions for Existing chat"),
      { button: 0 },
    );
  ...
  it("sorts chats by displayed title when A-Z is persisted", async () => {
    ...
    const group = within(sidebar).getByText("Tasks").closest("section");
    ...
    expect(labels).toEqual(["Alpha plan", "Start a task", "Zulu work"]);
  });
  ...
  it("restores sidebar run indicators after a page reload", async () => {
    ...
    localStorage.setItem(
      "pawbot-webui.sidebar.session-updates.v1",
  ...
  it("restores the active chat from the URL hash after a page reload", async () => {
    ...
    await waitFor(() => expect(connectSpy).toHaveBeenCalled());
    await waitFor(() => expect(document.title).toBe("Active after reload · pawbot"));
    const sidebar = screen.getByRole("navigation", { name: "Sidebar navigation" });
    expect(
  ...
  it("opens the settings view from the sidebar footer", async () => {
    ...
    localStorage.setItem(
      "pawbot-webui.settings-preferences",
    ...
    await waitFor(() => expect(connectSpy).toHaveBeenCalled());
    const sidebar = screen.getByRole("navigation", { name: "Sidebar navigation" });
    const searchButton = within(sidebar).getByRole("button", { name: "Find sessions" });
    const appsButton = within(sidebar).getByRole("button", { name: "Toolbox" });
    expect(searchButton.compareDocumentPosition(appsButton) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
    await user.click(within(sidebar).getByRole("button", { name: "Preferences" }));

    expect(
      await screen.findByRole("navigation", { name: "Preference sections" }),
    ).toBeInTheDocument();
    expect(screen.queryByRole("heading", { name: "Overview" })).not.toBeInTheDocument();
    expect(document.title).toBe("Preferences · pawbot");
    ...
    expect(screen.queryByTestId("overview-logo-pawbot-gateway")).not.toBeInTheDocument();
    expect(screen.queryByTestId("overview-logo-pawbot-workspace")).not.toBeInTheDocument();
    expect(screen.queryByRole("navigation", { name: "Sidebar navigation" })).not.toBeInTheDocument();
    const settingsNav = screen.getByRole("navigation", { name: "Preference sections" });
    expect(settingsNav.className).not.toContain("overflow-x-auto");
    expect(within(settingsNav).getByRole("button", { name: "Preferences: Overview" })).toBeInTheDocument();
    ...
    expect(within(settingsNav).queryByRole("button", { name: "Toolbox" })).not.toBeInTheDocument();
    expect(within(settingsNav).getByRole("button", { name: "Security" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Sign out" })).toBeInTheDocument();
    fireEvent.pointerDown(within(settingsNav).getByRole("button", { name: "Preferences: Overview" }));
    ...
    expect(within(settingsNav).getByRole("button", { name: "Preferences: Appearance" })).toBeInTheDocument();
    fireEvent.pointerDown(within(settingsNav).getByRole("button", { name: "Preferences: Appearance" }));
    ...
    expect(
      screen.queryByText(
        "Bring your own provider keys. Pawbot reads these values from the current config and only configured providers can be used in model presets.",
      ),
    ).not.toBeInTheDocument();
    ...
    expect(
      screen.queryByText("Restart pawbot to apply runtime changes."),
    ).not.toBeInTheDocument();
  ...
  it("keeps the backend timezone without writing settings on mount", async () => {
    ...
    const restartButton = system.getByRole("button", { name: "Restart pawbot" });
    expect(
      timezoneLabel.compareDocumentPosition(restartButton) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
    expect(
      system.queryByText("Restart pawbot to apply runtime changes."),
    ).not.toBeInTheDocument();
  });
  ...
  it("transitions between Apps and Skills without replacing the sidebar", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Toolbox" })).toBeInTheDocument();
    expect(screen.queryByText("Add tools to pawbot, then @ them in chat.")).not.toBeInTheDocument();
    expect(screen.getByRole("navigation", { name: "Sidebar navigation" })).toBeInTheDocument();
    expect(screen.queryByRole("navigation", { name: "Preference sections" })).not.toBeInTheDocument();
    expect(within(sidebar).getByRole("button", { name: "Toolbox" })).toHaveAttribute(
      "aria-current",
      "page",
    );
    expect(within(sidebar).getByTestId("utilities-selection-highlight")).toHaveAttribute(
      "data-active-id",
      "utility:apps",
    );
    ...
    expect(document.title).toBe("Toolbox · pawbot");
  ...
  it("returns from settings to the blank start page when no session was active", async () => {
    ...
    const sidebar = screen.getByRole("navigation", { name: "Sidebar navigation" });
    fireEvent.click(within(sidebar).getByRole("button", { name: "Start a task" }));
    await waitFor(() => expect(document.title).toBe("pawbot"));
  ...
  it("filters sessions in the centered search dialog", async () => {
    ...
    const newChatButton = within(sidebar).getByRole("button", { name: "Start a task" });
    const searchButton = within(sidebar).getByRole("button", { name: "Find sessions" });
    expect(
    ...
    const dialog = await screen.findByRole("dialog", { name: "Find sessions" });
    ...
    fireEvent.change(within(dialog).getByRole("textbox", { name: "Find sessions" }), {
    ...
    await waitFor(() =>
      expect(screen.queryByRole("dialog", { name: "Find sessions" })).not.toBeInTheDocument(),
    );
  });
  ...
  it("materializes a singleton tab without linking it to another pane", async () => {
    ...
    fireEvent.pointerDown(within(sidebar).getByRole("button", {
      name: "Task actions for Solo pane",
    }), { button: 0, ctrlKey: false });
  ...
  it("keeps panes and layout scoped to the current topic tab", async () => {
    ...
    const paneTopicButton = within(sidebar)
      .getAllByRole("button", { name: "Start a task" })
    ...
    fireEvent.pointerDown(within(sidebar).getByRole("button", {
      name: "Start a task pane actions",
    }), { button: 0, ctrlKey: false });
    fireEvent.click(screen.getByRole("menuitem", {
      name: "Remove",
    }));
    await waitFor(() => expect(screen.getByTestId("pane-grid").children).toHaveLength(1));
    expect(within(sidebar).getAllByRole("button", { name: "Start a task" })).toHaveLength(2);
  });
  ...
  it("opens search from the keyboard shortcut", async () => {
    ...
    const textbox = within(dialog).getByRole("textbox", { name: "Find sessions" });
  ...
  it("closes search when starting a new chat from the keyboard shortcut", async () => {
    ...
    await waitFor(() => expect(connectSpy).toHaveBeenCalled());
    fireEvent.keyDown(window, { key: "k", metaKey: true });
    expect(await screen.findByRole("dialog", { name: "Find sessions" })).toBeInTheDocument();
  ...
  it("uses macOS shortcut glyphs in the sidebar title", async () => {
    ...
    expect(within(sidebar).getByRole("button", { name: "Start a task" })).toHaveAttribute(
      "title",
      "Start a task (⌘⇧O)",
    );
  });
  ...
  it("keeps large sidebars light while search still covers every chat", async () => {
    ...
    fireEvent.click(within(sidebar).getByRole("button", { name: "Find sessions" }));
    const dialog = await screen.findByRole("dialog", { name: "Find sessions" });
    fireEvent.change(within(dialog).getByRole("textbox", { name: "Find sessions" }), {
      target: { value: "hidden" },
    });
  ...
  it("opens a blank start page without creating an empty chat", async () => {
    ...
    expect(within(rail).getByRole("button", { name: "Start a task" })).toBeInTheDocument();
    expect(within(rail).getByRole("button", { name: "Find sessions" })).toBeInTheDocument();
    ...
    expect(within(sidebar).getByRole("button", { name: "Preferences" })).toBeInTheDocument();
