...
async function request<T>(
  ...
  if (contentType && !contentType.toLowerCase().includes("application/json")) {
    ...
    throw new ApiError(
      res.status,
      isHtml
        ? "Gateway returned WebUI HTML instead of JSON. Restart pawbot gateway and try again."
...
export async function fetchPawbotFeatures(
  token: string,
  base: string = "",
): Promise<PawbotFeaturesPayload> {
  return request<PawbotFeaturesPayload>(
    `${base}/api/settings/pawbot-features`,
    token,
    undefined,
    API_READ_TIMEOUT_MS,
  );
...
export async function enablePawbotFeature(
  transport: WebUIMutationTransport,
  name: string,
  options: { instanceId?: string } = {},
): Promise<PawbotFeaturesPayload> {
  return mutation<PawbotFeaturesPayload>(
    transport,
...
export async function disablePawbotFeature(
  transport: WebUIMutationTransport,
  name: string,
  options: { instanceId?: string } = {},
): Promise<PawbotFeaturesPayload> {
...
export async function fetchReasoningEffortValues(
  ...
  const payload = await request<ReasoningEffortValuesPayload>(
    `${base}/api/settings/reasoning-effort-values?${query}`,
    token,
    undefined,
    API_READ_TIMEOUT_MS,
  );
...
export async function updateSettings(
  ...
  if (update.reasoningEffort !== undefined) {
    payload.reasoning_effort = update.reasoningEffort ?? "";
  }
...
// --- Blackbox (Record & Replay) + token governance -------------------------
...
export interface BlackboxBreakpoint {
  directory: string;
  breakpoint: true;
  iteration: number;
  turn_id: string | null;
...
export interface BlackboxTokens {
...
export async function blackboxStatus(
  ...
  return mutation<BlackboxStatus>(transport, "blackbox.status");
}

export async function blackboxStart(
  transport: WebUIMutationTransport,
  directory: string,
): Promise<{ recording: boolean; directory: string }> {
  return mutation(transport, "blackbox.start", { directory });
}

export async function blackboxStop(
  transport: WebUIMutationTransport,
): Promise<{ recording: boolean }> {
  return mutation(transport, "blackbox.stop");
}

export async function blackboxDelete(
  transport: WebUIMutationTransport,
  directory: string,
): Promise<{ deleted: boolean; directory: string }> {
  return mutation(transport, "blackbox.delete", { directory });
}
...
export async function blackboxDetail(
  ...
  directory: string,
  turnId: string,
): Promise<BlackboxDetail> {
  return mutation<BlackboxDetail>(transport, "blackbox.detail", {
    directory,
    turn_id: turnId,
  });
}

export async function blackboxReplay(
  transport: WebUIMutationTransport,
  directory: string,
  breakAt?: number,
): Promise<BlackboxReplayResult | BlackboxBreakpoint> {
  return mutation<BlackboxReplayResult | BlackboxBreakpoint>(
    transport,
    "blackbox.replay",
    { directory, break_at: breakAt ?? null },
  );
}

export async function blackboxTokens(
  transport: WebUIMutationTransport,
  sessionKey?: string | null,
): Promise<BlackboxTokens> {
