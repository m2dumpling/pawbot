...
const WS_OPEN = 1;
const WS_CLOSING = 2;
const HOST_SOCKET_URL_PREFIX = "pawbot-host://";
 ...
 * - **Production**: off unless ``localStorage.setItem('pawbot_debug_ws','1')`` (or ``true``).
 * - **Silence anywhere**: ``localStorage.setItem('pawbot_debug_ws','0')`` (or ``false`` / ``off``).
...
function wsInboundDebugEnabled(): boolean {
  ...
  try {
    ...
    const raw = ls?.getItem("pawbot_debug_ws")?.trim().toLowerCase() ?? "";
...
export interface PawbotClientOptions {
  url: string;
  reconnect?: boolean;
...
export class PawbotClient {
  ...
  constructor(private options: PawbotClientOptions) {
  ...
  reconcileCanonicalCompletion(
    ...
    while (fences.size > PawbotClient.COMPLETED_TURN_FENCE_MAX) {
  ...
  private handleMessage(ev: MessageEvent): void {
    ...
    try {
      ...
      if (wsInboundDebugEnabled()) {
        ...
        console.warn(
          "[pawbot ws inbound] invalid JSON",
  ...
  private dispatch(chatId: string, ev: InboundEvent): void {
    ...
    q.push(ev);
    const over = q.length - PawbotClient.PENDING_INBOUND_MAX;
    if (over > 0) {
      q.splice(0, over);
    }
  }
