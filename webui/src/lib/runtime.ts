...
const LOOPBACK_HOST_STORAGE_KEY = "pawbot-webui.native-host";
...
let loopbackHostApi: PawbotHostApi | null = null;

declare global {
  interface Window {
    pawbotHost?: PawbotHostApi;
  }
}

function getHostApi(): PawbotHostApi | null {
  if (typeof window === "undefined") return null;
  return window.pawbotHost ?? loopbackHostApi;
}
...
function createLoopbackHostApi(config: LoopbackHostConfig): PawbotHostApi {
  return {
    async pickFolder(): Promise<string | null> {
      ...
      try {
        ...
        throw new Error("Native folder picker is unavailable. Reopen Pawbot and try again.");
      }
