...
export function SettingsPage({
  ...
  const {
    ...
    pawbotFeatureAction,
    pawbotFeatureConfirm,
    pawbotFeatures,
    pawbotFeaturesError,
    pawbotFeaturesLoading,
    networkSafetyDirty,
    networkSafetyForm,
    networkSafetySaving,
      ...
      case "models":
        return (
          <div className="space-y-8">
            ...
            <ProvidersSettings
              settings={settings}
              pawbotFeatures={pawbotFeatures}
              featureAction={pawbotFeatureAction}
      ...
      case "browser":
        return (
          <WebSettings
            ...
            olostepInstalling={pawbotFeatureAction === "enable:olostep"}
            capabilityError={pawbotFeaturesError}
          />
        );
      case "channels":
        return (
          <ChannelsSettings
            token={token}
            pawbotFeatures={pawbotFeatures}
            loading={pawbotFeaturesLoading}
            query={channelsQuery}
            actionKey={pawbotFeatureAction}
            chatAppsDocsUrl={settings.docs?.chat_apps_url}
            showBrandLogos={localPrefs.brandLogos}
            error={pawbotFeaturesError}
            requiresRestartPending={pendingRestartSections.runtime}
            onQueryChange={setChannelsQuery}
            onAction={handlePawbotFeatureAction}
            onFeaturesUpdate={setPawbotFeatures}
            onDismissStatus={() => {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-settings-canvas lg:flex-row">
      ...
      <PawbotFeatureInstallDialog
        feature={pawbotFeatureConfirm}
        installing={pawbotFeatureAction === `enable:${pawbotFeatureConfirm?.name ?? ""}`}
        onOpenChange={(open) => {
          if (!open) setPawbotFeatureConfirm(null);
        }}
        onConfirm={(feature) => handlePawbotFeatureAction("enable", feature.name, true)}
      />
