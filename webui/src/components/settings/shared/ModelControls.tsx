...
import type { ProviderModelInfo, ProviderModelsPayload, SettingsPayload } from "@/lib/types";
...
export function ModelIdPicker({
  ...
  emptyMessage?: string;
  onChange: (model: string, info?: ProviderModelInfo) => void;
}) {
  ...
  const selectModel = (model: string) => {
    onChange(model, providerModels.find((candidate) => candidate.id === model));
    setOpen(false);
  };
    ...
    >
      <span className="flex min-w-0 items-center gap-2">
        ...
        <span className="min-w-0">
          ...
          {model.reasoning_effort_values?.length ? (
            <span className="mt-0.5 block truncate text-[10.5px] text-muted-foreground">
              {tx("settings.models.reasoningLevels", "Thinking")}: {model.reasoning_effort_values
