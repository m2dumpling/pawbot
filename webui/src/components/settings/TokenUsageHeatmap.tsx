...
export function TokenUsageHeatmap({
  ...
  const hasActivity = cells.some((cell) => cell.total > 0 || cell.requests > 0);

  if (!hasActivity) {
    return (
      <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-4 py-5 sm:px-5">
        <div className="flex items-center justify-between gap-3">
          <span className="text-xs font-medium text-foreground/80">
            {tx("settings.usage.shortTitle", "Token Usage")}
          </span>
          <span className="text-[11px] text-muted-foreground/70">
            {tx("settings.usage.empty", "Token activity will appear after new model replies.")}
          </span>
        </div>
      </div>
    );
