...
import type { ChannelRuntimeStatus, PawbotFeatureInfo } from "@/lib/types";
...
export function channelSetup(
  ...
  const presentation: ChannelSetupPresentation = {
    ...
    summary:
      ...
      ?? "Enable turns on this channel in pawbot, but this integration still needs platform-specific setup before it can receive messages.",
    tryIt: setupMessages?.tryIt,
    steps: setupMessages?.steps ?? [
      `Open ~/.pawbot/config.json and find channels.${feature.name}.`,
      "Add the credentials required by that platform, using the channel documentation as the source of truth.",
      "Restart pawbot, then send a small test message from that platform.",
    ],
...
function channelDisplayName(feature: PawbotFeatureInfo): string {
...
export function channelDescription(feature: PawbotFeatureInfo, t: ReturnType<typeof useTranslation>["t"]): string {
  const fallback =
    `Use pawbot from ${channelDisplayName(feature)}.`;
  return channelTranslator(t, channelUiOwner(feature.name))("description", fallback);
}

export function channelRequirements(feature: PawbotFeatureInfo, t: ReturnType<typeof useTranslation>["t"]): string {
  const fallback =
    "Channel credentials and gateway settings";
  return channelTranslator(t, channelUiOwner(feature.name))("requirements", fallback);
}

export function channelMatchesFilter(feature: PawbotFeatureInfo, filter: ChannelFilter): boolean {
  if (filter === "on") return channelIsRunning(feature);
  if (filter === "off") return !channelIsRunning(feature);
  return true;
}

export function channelIsRunning(feature: PawbotFeatureInfo): boolean {
  return feature.runtime_status === "running";
}

export function channelToggleChecked(feature: PawbotFeatureInfo): boolean {
