...
export function ChannelSetupPanel({
  ...
  const installSupportLabel = tx("settings.pawbotFeatures.installSupport", "Install support");
...
function ChannelSetupSurface({
  ...
  const summary = setup.summary ?? tx(
    ...
    "Enable only turns on pawbot support. Add the platform credentials, then restart pawbot.",
  );
