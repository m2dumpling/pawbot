...
export type ChannelInstancesPanelCustomization = {
  ...
  toggleAriaLabel?: (instance: PawbotChannelInstanceInfo) => string;
  configuredLabel?: string;
  needsSetupLabel?: string;
  renderInstanceSummary?: (instance: PawbotChannelInstanceInfo) => ReactNode;
  renderInstanceAction?: (instance: PawbotChannelInstanceInfo) => ReactNode;
  footer?: ReactNode;
};
...
export function ChannelInstancesPanel({
  ...
  instances?: PawbotChannelInstanceInfo[];
  ...
  const toggleInstance = async (instance: PawbotChannelInstanceInfo, checked: boolean) => {
    setBusyInstanceId(instance.id);
    setNotice(null);
    try {
      const payload = checked
        ? await enablePawbotFeature(client, feature.name, { instanceId: instance.id })
        : await disablePawbotFeature(client, feature.name, { instanceId: instance.id });
  ...
  const saveSelectedInstanceSettings = async () => {
    ...
    try {
      ...
      if (payload.pawbot_features) {
        onFeaturesUpdate(payload.pawbot_features);
      }
...
function channelInstanceDisplayName(instance: PawbotChannelInstanceInfo): string {
...
function instanceToggleChecked(instance: PawbotChannelInstanceInfo): boolean {
...
function ChannelInstanceStatusBadge({
  ...
  instance: PawbotChannelInstanceInfo;
  configuredLabel?: string;
  needsSetupLabel?: string;
}) {
