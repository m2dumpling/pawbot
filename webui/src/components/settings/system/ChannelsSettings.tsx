...
import type { PawbotFeaturesPayload } from "@/lib/types";
...
export function ChannelsSettings({
  ...
  token: string;
  pawbotFeatures: PawbotFeaturesPayload | null;
  loading: boolean;
  query: string;
  ...
  const allChannels = (pawbotFeatures?.features ?? [])
  ...
  return (
    ...
    >
      ...
      {requiresRestartPending ? (
        <div className="mt-3 shrink-0">
          <RestartRequiredNotice
            message={tx("settings.channels.restartRequired", "Restart pawbot to apply updated channel support.")}
            onRestart={onRestart}
