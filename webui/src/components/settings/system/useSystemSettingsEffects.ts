...
export function useSystemSettingsEffects({
  ...
  useEffect(() => {
    ...
    const refresh = async (showLoading = false): Promise<void> => {
      ...
      refreshing = true;
      if (showLoading) setPawbotFeaturesLoading(true);
      try {
        const payload = await fetchPawbotFeatures(getToken());
        if (!cancelled) {
        ...
        if (!cancelled && message !== "HTTP 404") setPawbotFeaturesError(message);
      } finally {
        refreshing = false;
        if (!cancelled && showLoading) setPawbotFeaturesLoading(false);
      }
    };
    void refresh(true);
