...
import type { PawbotClient } from "@/lib/pawbot-client";
import type {
...
interface SystemSettingsActionsOptions {
  ...
  featureCatalog: PawbotFeatureInfo[];
  client: PawbotClient;
  token: string;
  getToken: () => string;
  t: TFunction;
...
export function createSystemSettingsActions({
  ...
  const {
    ...
    setMcpPresetAction,
    setMcpPresets,
    setPawbotFeatureAction,
    setPawbotFeatureConfirm,
    setPawbotFeatures,
    setPawbotFeaturesError,
  } = state;
  ...
  const installCapabilities = async (names: string[]): Promise<boolean> => {
    ...
    setPawbotFeatureAction(`enable:${names.join("+")}`);
    setPawbotFeaturesError(null);
    try {
      let latest = pawbotFeatures;
      for (const name of missing) {
        latest = await enablePawbotFeature(client, name);
        if (latest.requires_restart) {
          setPendingRestartSections((prev) => ({ ...prev, runtime: true }));
        }
      }
      if (latest) setPawbotFeatures(latest);
      return true;
    } catch (err) {
      setPawbotFeaturesError((err as Error).message);
      return false;
    } finally {
      setPawbotFeatureAction(null);
    }
  };
  ...
  const handleApiServiceAction = async (
    ...
    try {
      ...
      const refreshed = await fetchPawbotFeatures(token);
      setPawbotFeatures(refreshed);
  ...
  const handlePawbotFeatureAction = async (
    ...
    if (action === "enable" && !confirmed && feature && !feature.installed && feature.install_supported) {
      setPawbotFeaturesError(null);
      setPawbotFeatureConfirm(feature);
      return;
    }
    const key = `${action}:${name}`;
    setPawbotFeatureAction(key);
    setPawbotFeatureConfirm(null);
    setPawbotFeaturesError(null);
    try {
      const payload = action === "enable"
        ? await enablePawbotFeature(client, name)
        : await disablePawbotFeature(client, name);
      setPawbotFeatures(payload);
    ...
    if (flow.status === "authorized" && flow.hot_reload) {
      ...
      setMcpError(
        ...
        || t("settings.mcp.reloadFailed", {
          defaultValue: "Signed in, but pawbot could not connect the tools. Try restarting pawbot.",
        }),
      );
      return;
    }
