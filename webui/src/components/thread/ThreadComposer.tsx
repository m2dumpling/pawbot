...
import type { SendAttachment, SendOptions } from "@/hooks/usePawbotStream";
...
const SLASH_RECENTS_STORAGE_KEY = "pawbot.webui.slashCommandRecents";
const SLASH_RECENTS_LIMIT = 5;
const QUEUED_PROMPTS_STORAGE_PREFIX = "pawbot.webui.composerQueuedGuidance.v1:";
  ...
  return (
    ...
    >
      {goalPanelOpen && canExpandGoal && markdownBody ? (
        <div
          ref={panelRef}
          id="pawbot-goal-panel-root"
          role="dialog"
          aria-modal="false"
          aria-labelledby="pawbot-goal-panel-title"
          tabIndex={-1}
          className={cn(
          ...
          >
            ...
            {canExpandGoal ? (
              <button
                ...
                aria-controls={goalPanelOpen ? "pawbot-goal-panel-root" : undefined}
...
export function ThreadComposer({
  ...
  const inputTextClasses = cn(
    ...
    isHero
      ? cn(
          "pawbot-composer-input min-h-[92px] px-5 text-[16px] leading-6",
          relaxedHeroInput ? "pb-3 pt-[30px]" : "pb-2 pt-5",
        )
      : "pawbot-composer-input min-h-[64px] px-4 pb-2 pt-4 text-[16px] leading-5",
  );
    ...
    >
      ...
      <div
        ...
        className={cn(
          ...
          isHero
            ? "pawbot-composer-deck pawbot-composer-deck-hero max-w-[64rem] rounded-[1.5rem] border border-border/75 bg-card shadow-[0_18px_50px_-34px_hsl(var(--foreground)/0.45)] focus-within:border-primary/30 focus-within:shadow-[0_20px_55px_-32px_hsl(var(--primary)/0.35)]"
            : "pawbot-composer-deck max-w-[56rem] rounded-[1.35rem] border border-border/75 bg-card shadow-[0_12px_36px_-28px_hsl(var(--foreground)/0.4)] focus-within:border-primary/30 focus-within:shadow-[0_16px_42px_-26px_hsl(var(--primary)/0.28)]",
      ...
      >
        ...
        <div
          className={cn(
            "thread-composer-footer flex flex-nowrap items-center border-t border-border/55 bg-muted/[0.16] motion-safe:transition-[padding-bottom] motion-safe:[transition-duration:220ms] motion-safe:ease-in-out",
            isHero
              ? cn(
                  "gap-x-1.5 rounded-b-[1.5rem] px-4",
                  showProjectPicker ? "pb-1.5 pt-2" : "pb-3 pt-2",
                )
              : "gap-x-2 rounded-b-[1.35rem] px-3 pb-2.5 pt-2",
          )}
        >
          <div
            className={cn(
          ...
          >
            ...
            <Button
              ...
              className={cn(
                "thread-composer-action touch-target rounded-[0.7rem] text-muted-foreground hover:text-foreground",
                isHero
                  ? "h-8 w-8 border border-border/65 bg-background shadow-[0_2px_8px_hsl(var(--foreground)/0.04)] hover:border-primary/25 hover:bg-primary/[0.05]"
                  : "h-9 w-9 border border-border/65 bg-background shadow-[0_2px_8px_hsl(var(--foreground)/0.04)] hover:border-primary/25 hover:bg-primary/[0.05]",
              )}
            >
          ...
          >
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      className={cn(
                        "thread-composer-action touch-target rounded-[0.7rem] border border-transparent text-muted-foreground hover:bg-muted/65 hover:text-foreground",
            ...
            <Button
              ...
              className={cn(
                "thread-composer-action touch-target rounded-[0.7rem] transition-transform",
                showStopButton
                  ? "border border-border/70 bg-background text-foreground/85 shadow-[0_3px_10px_hsl(var(--foreground)/0.07)] hover:bg-muted/65 hover:text-foreground disabled:text-muted-foreground/50"
                  : isHero
                    ? "border border-primary bg-primary text-primary-foreground shadow-[0_5px_14px_hsl(var(--primary)/0.28)] hover:bg-primary/90 disabled:border-primary disabled:bg-primary disabled:text-primary-foreground"
                    : "border border-primary bg-primary text-primary-foreground shadow-[0_4px_12px_hsl(var(--primary)/0.24)] hover:bg-primary/90 disabled:border-primary disabled:bg-primary disabled:text-primary-foreground",
                isHero ? "h-8 w-8" : "h-9 w-9",
