...
import { Check, ChevronDown, Folder, Hand, KeyRound } from "lucide-react";
      ...
      <DropdownMenuTrigger asChild disabled={disabled || !onChange}>
        <Button
          ...
          className={cn(
            ...
            isFull
              ? "border-violet-300/60 bg-violet-50/75 text-violet-700 hover:border-violet-400/70 hover:bg-violet-100/70 dark:border-violet-400/35 dark:bg-violet-500/10 dark:text-violet-300 dark:hover:bg-violet-400/10"
              : "border-border/65 bg-background/70 text-muted-foreground hover:border-primary/20 hover:bg-muted/55 hover:text-foreground dark:hover:bg-white/[0.06]",
          )}
        >
          {isFull ? (
            <KeyRound className={cn("thread-composer-access-icon mr-1.5 shrink-0", isHero ? "h-3.5 w-3.5" : "h-3.5 w-3.5")} />
          ) : (
  ...
  return (
    <DropdownMenuItem
      ...
      className={cn(
        ...
        warning && "text-violet-700 focus:text-violet-700 dark:text-violet-300 dark:focus:text-violet-300",
      )}
    >
