...
import { usePawbotStream, type SendAttachment, type SendOptions } from "@/hooks/usePawbotStream";
...
import type { CanonicalRunSnapshot, StreamError } from "@/lib/pawbot-client";
  ...
  return settings.model_presets
    ...
    .map((preset) => {
      ...
      return {
        ...
        reasoningEffortValues: preset.reasoning_effort_values ?? undefined,
      };
    });
}
...
function HeroGreeting({ text }: { text: string }) {
  ...
  return (
    <div ref={containerRef} className="min-w-0 w-full max-w-[44rem]">
      ...
      >
        <PawPrintMark className="h-8 w-8" />
...
function PawPrintMark({ className }: { className?: string }) {
  return (
    <svg
      ...
      aria-hidden
      className={cn(
        "select-none text-violet-600 drop-shadow-[0_0_12px_hsl(263_85%_60%/0.35)] dark:text-violet-400",
        className,
      )}
    >
      <g>
        <ellipse cx="14" cy="15" rx="4.6" ry="5.2" fill="currentColor" />
        <ellipse cx="24" cy="12" rx="4.6" ry="5.2" fill="currentColor" />
        <ellipse cx="34" cy="15" rx="4.6" ry="5.2" fill="currentColor" />
        <ellipse cx="10.5" cy="26" rx="4" ry="4.6" fill="currentColor" />
        <ellipse cx="37.5" cy="26" rx="4" ry="4.6" fill="currentColor" />
        <path
          d="M24 21.5c5.4 0 10.2 3.9 10.2 9.2 0 4.4-3.6 6.9-6.2 6.9-1.3 0-2.2-.7-4-.7s-2.7.7-4 .7c-2.6 0-6.2-2.5-6.2-6.9 0-5.3 4.8-9.2 10.2-9.2Z"
          fill="currentColor"
        />
      </g>
    </svg>
...
export function ThreadShell({
  ...
  const {
    ...
    streamError,
    dismissStreamError,
  } = usePawbotStream(chatId, initial, hasPendingToolCalls, handleTurnEnd);
  ...
  const activePresetRow = useMemo(
    () => settings?.model_presets.find((preset) => preset.name === activeModelPreset) ?? null,
    [activeModelPreset, settings],
  );
  const [localReasoningEffort, setLocalReasoningEffort] = useState<string | null>(null);
  const [probedEffortValues, setProbedEffortValues] = useState<string[] | null>(null);
  useEffect(() => {
    setLocalReasoningEffort(null);
    setProbedEffortValues(null);
    if (activePresetRow?.reasoning_effort_values?.length) return;
    if (!modelName) return;
    const provider = activePresetRow?.resolved_provider || activePresetRow?.provider || "";
    let cancelled = false;
    fetchReasoningEffortValues(getToken(), provider, modelName)
      .then((values) => {
        if (!cancelled) setProbedEffortValues(values);
      })
      .catch(() => {
        if (!cancelled) setProbedEffortValues([]);
      });
    return () => {
      cancelled = true;
    };
  }, [activePresetRow, getToken, modelName]);
  const activeReasoningEffort = (
    localReasoningEffort
    ?? activePresetRow?.reasoning_effort
    ?? settings?.agent.reasoning_effort
    ?? ""
  );
  const activeReasoningEffortValues = (
    probedEffortValues
    ?? activePresetRow?.reasoning_effort_values
    ?? []
  );
  ...
  const handleReasoningEffortChange = useCallback(async (effort: string) => {
    const next = effort === "" ? null : effort;
    setLocalReasoningEffort(next);
    if (!activePresetRow || activePresetRow.is_default) {
      try {
        await updateSettings(client, { reasoningEffort: next });
    ...
    try {
      await updateModelConfiguration(client, {
        name: activePresetRow.name,
  ...
  }, [activePresetRow, client, refreshModelSettings]);
  ...
  // Persist thread to in-memory cache after paint so ``usePawbotStream``'s chat switch
    ...
    <>
      ...
      {session ? (
        <ThreadComposer
          ...
          reasoningEffortValues={activeReasoningEffortValues}
  ...
  const emptyState = loading ? (
    ...
    <div className="pawbot-welcome-panel flex w-full flex-col items-center text-center animate-in fade-in-0 slide-in-from-bottom-2 [animation-duration:220ms] motion-reduce:animate-none">
      <div className="mb-4 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-primary/75">
