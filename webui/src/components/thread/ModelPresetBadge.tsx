...
export function ModelPresetBadge({
  ...
  const effortValue = reasoningEffort ?? activePreset.reasoningEffort ?? "";
  const effortValues = reasoningEffortValues ?? activePreset.reasoningEffortValues ?? [];
  ...
  return (
    ...
    >
      ...
      >
        ...
        {effortValues.length > 0 && onReasoningEffortChange ? (
          <div className="mt-1 border-t border-border/55 pt-1.5">
            <p className="px-2.5 pb-1.5 text-[11px] font-medium text-muted-foreground">
              {t("thread.composer.reasoningEffort.label")}
            </p>
            <div className="flex flex-wrap gap-1 px-2 pb-1">
              {effortValues.map((value) => {
                const selected = effortValue === value;
                const label = value === ""
                  ? t("thread.composer.reasoningEffort.default")
                  : value;
                return (
                  <button
                    key={value || "default"}
                    ...
                    aria-label={label}
                    onClick={() => {
                      if (!selected) onReasoningEffortChange(value);
                    }}
                    className={cn(
                      floatingItemClassName,
                      floatingItemFocusClassName,
                      "h-7 min-w-10 cursor-pointer rounded-lg px-2.5 text-[12px] font-medium",
                      selected
...
function PresetPill({
  ...
  return (
    <span
      ...
      className={cn(
        "composer-model-badge composer-model-pill inline-flex h-full max-w-full min-w-0 shrink-0 items-center rounded-[0.7rem] border border-border/65 bg-background font-medium text-foreground/75 shadow-[0_2px_8px_hsl(var(--foreground)/0.035)]",
        "w-fit",
