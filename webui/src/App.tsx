...
import { PawbotClient } from "@/lib/pawbot-client";
...
const SIDEBAR_STORAGE_KEY = "pawbot-webui.sidebar";
const SESSION_UPDATES_STORAGE_KEY = "pawbot-webui.sidebar.session-updates.v1";
const LEGACY_COMPLETED_RUNS_STORAGE_KEY = "pawbot-webui.sidebar.completed-runs.v1";
const RESTART_STARTED_KEY = "pawbot-webui.restartStartedAt";
const RESTART_ROUTE_KEY = "pawbot-webui.restartRoute";
...
export default function App() {
  ...
  const refreshReadyClient = useCallback(
    async (client: PawbotClient, fallbackSurface: RuntimeSurface) => {
...
function Shell({
  ...
  const onRemoveWorkbenchPane = useCallback((paneKey: string) => {
    const match = workbenchTabForPane(workbenchState, paneKey);
    if (match && activeKey === paneKey) {
      const remaining = match.tab.paneKeys.filter((key) => key !== paneKey);
      const fallback = remaining[0];
      if (fallback) onSelectChat(fallback);
    }
    updateWorkbenchState((current) => {
      const found = workbenchTabForPane(current, paneKey);
      return found ? detachWorkbenchPane(current, found.tabKey, paneKey) : current;
    });
  }, [activeKey, onSelectChat, updateWorkbenchState, workbenchState]);
  ...
  useEffect(() => {
    ...
    if (view === "apps") {
      document.title = t("app.documentTitle.chat", {
        title: t("settings.nav.apps", { defaultValue: "Toolbox" }),
      });
      return;
    }
    if (view === "automations") {
    ...
    if (view === "skills") {
      document.title = t("app.documentTitle.chat", {
        title: t("settings.nav.skills", { defaultValue: "Prompt packs" }),
      });
      return;
    }
        ...
        >
            ...
            >
              <PaneWorkbench
                ...
                onAddPane={onAddPane}
                onRemovePane={onRemoveWorkbenchPane}
