...
function guardWebuiEntryChunk(): Plugin {
  return {
    name: "pawbot-guard-webui-entry-chunk",
    apply: "build",
...
export function webuiManualChunk(id: string): string | undefined {
  // Split only the root application surfaces. Their child components remain
  // in the shared entry, which avoids the circular chunks produced by grouping
  // entire component trees while preserving synchronous first-render behavior.
  if (id.endsWith("/src/components/Sidebar.tsx")) return "sidebar-ui";
  if (id.endsWith("/src/components/thread/ThreadShell.tsx")) return "thread-ui";
  if (id.endsWith("/src/components/workbench/PaneWorkbench.tsx")) return "workbench-ui";
  if (
...
export default defineConfig(({ mode }) => {
  ...
  const target = env.PAWBOT_API_URL ?? "http://127.0.0.1:8765";
  const hmrPath = "/__pawbot_vite_hmr";
  ...
  return {
    ...
    build: {
      outDir: path.resolve(__dirname, "../pawbot/web/dist"),
      emptyOutDir: true,
    ...
    server: {
      ...
      // Keep Vite's HMR socket on a dedicated path. Pawbot's app WebSocket is
    ...
    test: {
      ...
      globals: true,
      // Several UI suites share lazy markdown modules and happy-dom globals.
      // Serializing files keeps the default test command deterministic; tests
      // inside each file remain concurrent where Vitest permits it.
