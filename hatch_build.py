"""Hatch build hook that bundles the webui (Vite) into pawbot/web/dist.
...
- No-op when `webui/package.json` is absent (e.g. installing from an sdist that
  already contains a prebuilt `pawbot/web/dist/`).
- Skips when `PAWBOT_SKIP_WEBUI_BUILD=1` is set.
- Reuses `pawbot/web/dist/` only when it is already fresh, unless
  `PAWBOT_FORCE_WEBUI_BUILD=1` is set.
...
def _load_webui_build_module() -> ModuleType:
    from pawbot.webui import build as webui_build
...
class WebUIBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict) -> None:  # noqa: D401
        ...
        force = os.environ.get("PAWBOT_FORCE_WEBUI_BUILD") == "1"
        if not status.needs_build and not force:
            self.app.display_info(
                f"[webui-build] reusing existing build at {dist_dir} "
                "(already fresh; set PAWBOT_FORCE_WEBUI_BUILD=1 to rebuild)"
            )
            return
