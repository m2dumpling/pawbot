...
- pawbot TUI: the `tui/` directory in the GitHub release tag that contains this archive
...
`pawbot-tui-source.tar.gz` from the release archive and run the resulting Bun executable:

```bash
cd pawbot-tui-source
...
Questions about source availability should be reported through the issue tracker
of the repository that published the corresponding release archive.
