...
const options: AppOptions = {
  ...
  model: "test/model",
  modelPreset: "default",
  workspace: "/tmp/pawbot-workspace",
  version: "test",
...
test("formats a reusable session ID after exit", () => {
  expect(sessionExitMessage("resume-chat")).toBe(
    "Resume with: pawbot agent --session websocket:resume-chat\n",
  )
})
...
const mount = (setup: TestRendererSetup, sent: string[] = []) => PawbotTui.mount(
  setup.renderer,
  options,
  client(sent),
...
describe("PawbotTui layout", () => {
  ...
  test("reflows a single retained layout across terminal resizes", async () => {
    ...
    for (const [width, height] of [[100, 30], [56, 18], [118, 36]] as const) {
      ...
      expect(occurrences(frame, "Woof — give me a task")).toBe(1)
      expect(occurrences(frame, "Ready")).toBe(0)
      expect(occurrences(frame, "Getting ready…")).toBe(1)
      expect(occurrences(frame, "pawbot  ·  test/model")).toBe(1)
    }
  ...
  test("clears the placeholder on the first typed character", async () => {
    ...
    const composer = (app as unknown as { composer: TextareaRenderable }).composer
    await setup.renderOnce()
    expect(setup.captureCharFrame()).toContain("Woof — give me a task")
    ...
    expect(frame).not.toContain("Woof — give me a task")
    expect(frame).not.toContain("buWoof — give me a task")

    setup.mockInput.pressBackspace()
    setup.mockInput.pressBackspace()
    await setup.flush()
    expect(composer.placeholder).toBe("Woof — give me a task")
    expect(setup.captureCharFrame()).toContain("Woof — give me a task")
  })
  ...
  test("pastes clipboard images into removable placeholders and sends their data", async () => {
    ...
    expect(placeholderStyle?.fg?.toInts().slice(0, 3)).toEqual([167, 139, 250])
    ...
    expect(imageChunk?.fg?.toInts().slice(0, 3)).toEqual([167, 139, 250])
  ...
  test("switches and creates gateway chats without replacing core slash commands", async () => {
    ...
    const app = PawbotTui.mount(
      setup.renderer,
      { ...options, apiUrl: "http://pawbot.test", apiToken: "secret" },
      transport,
      new MockTreeSitterClient({ autoResolveTimeout: 0 }),
    )
    app.accept({ event: "attached", chat_id: "chat" })
  ...
  test("refreshes expired API credentials before opening sessions", async () => {
    ...
    const app = PawbotTui.mount(
      ...
      {
        ...options,
        bootstrapUrl: "http://pawbot.test/webui/bootstrap",
        bootstrapSecret: "bootstrap-secret",
        apiUrl: "http://pawbot.test",
  ...
  test("opens composer runtime controls by mouse and applies canonical choices", async () => {
    ...
    try {
      ...
      expect(scopes).toEqual([{
        project_path: "/tmp/pawbot-workspace",
        access_mode: "full",
  ...
  test("shows compact session context without exposing private reasoning", async () => {
    ...
    try {
      ...
      expect(occurrences(compact, "Woof — give me a task")).toBe(1)
  ...
  test("loads earlier transcript pages in place when PageUp reaches the top", async () => {
    ...
    const app = PawbotTui.mount(
      setup.renderer,
      { ...options, apiUrl: "http://pawbot.test", apiToken: "secret", chatId: "chat" },
      client(),
      new MockTreeSitterClient({ autoResolveTimeout: 0 }),
    )
  ...
  test("survives rapid narrow resizes with long CJK and code", async () => {
    ...
    app.accept({
      ...
      text: [
        ...
        "https://pawbot.test/a-very-long-unbroken-path-that-must-not-break-the-layout",
        "```ts",
        "const greeting = '你好，pawbot'",
        "```",
      ].join("\n\n"),
    })
    app.accept({ event: "stream_end", chat_id: "chat" })
    ...
    for (const [width, height] of [
      ...
      expect(occurrences(frame, "pawbot  ·  test/model")).toBe(height >= 14 ? 1 : 0)
    }
  })
  ...
  test("inherits the host background after long output fills the viewport", async () => {
    ...
    app.accept({
      ...
      text: Array.from({ length: 80 }, (_, index) => (
        `### Section ${index + 1}\n中文长回答、**bold** and [link](https://pawbot.test/${index + 1})`
      )).join("\n\n"),
    })
    app.accept({ event: "stream_end", chat_id: "chat" })
    app.accept({ event: "turn_end", chat_id: "chat" })
  ...
  test("distinguishes the composer with a quiet focus edge", async () => {
    ...
    expect(internals.composerFrame.borderColor.toInts().slice(0, 3)).toEqual([124, 58, 237])
    expect(internals.composerFrame.backgroundColor.toInts().slice(0, 3)).toEqual([240, 240, 240])
    expect(internals.composer.backgroundColor.toInts().slice(0, 3)).toEqual([240, 240, 240])
    ...
    const composerLine = setup.captureCharFrame().split("\n")
      .find((line) => line.includes("Woof — give me a task")) || ""
  ...
  test("uses asymmetric roles instead of chat bubbles", async () => {
    ...
    const headerLine = frame.split("\n").find((line) => line.includes(">_  pawbot")) || ""
  ...
  test("keeps semantic colors legible in both terminal appearances", async () => {
    ...
    assertContrast()
    expect(internals.palette.accent).toBe("#A78BFA")
    expect(internals.palette.user).toBe("#A78BFA")
    setup.renderer.emit(CliRenderEvents.THEME_MODE, "light")
    assertContrast()
    expect(internals.palette.accent).toBe("#7C3AED")
    expect(internals.palette.user).toBe("#7C3AED")
  })
  ...
  test("replaces streamed drafts with canonical stream-end text", async () => {
    ...
    app.accept({
      event: "stream_end",
      chat_id: "chat",
      text: "canonical https://pawbot.test/signed/current",
      resuming: true,
      merge_next: true,
    })
    ...
    app.accept({
      event: "stream_end",
      chat_id: "chat",
      text: "final https://pawbot.test/signed/current tail",
    })
    app.accept({ event: "turn_end", chat_id: "chat" })
    await setup.flush()
    const frame = setup.captureCharFrame()

    expect(frame).toContain("final https://pawbot.test/signed/current tail")
    expect(frame).not.toContain("canonical https://pawbot.test/signed/current")
  ...
  test("animates one stable status line while the agent works", async () => {
    ...
    expect([...shimmerColors].some((value) => value?.startsWith("167,139,250"))).toBe(true)
    ...
    app.accept({ event: "turn_end", chat_id: "chat" })
    await setup.flush()
    expect(ui.composer.placeholder).toBe("Woof — give me a task")
  })
  ...
  test("groups consecutive file activity only in the collapsed preview", async () => {
    ...
    app.accept({
      ...
      tool_events: Array.from({ length: 6 }, (_, index) => ({
        ...
        call_id: `read-${index}`,
        name: "read_file",
        arguments: { path: `/tmp/pawbot-workspace/src/file-${index}.ts` },
      })),
    })
    await setup.renderOnce()
    let frame = setup.captureCharFrame()
  ...
  test("reconciles active state from attach hydration after reconnect", async () => {
    ...
    expect(occurrences(restored, ">_  pawbot")).toBe(1)
    app.accept({
      event: "goal_status",
      chat_id: "chat",
      status: "running",
  ...
  test("shows actionable connection states without implementation details", async () => {
    ...
    expect(ui.status.plainText).toBe("Pawbot is taking longer to respond…")
    expect(ui.status.plainText).not.toContain("Unable")
    ...
    expect(ui.status.plainText).toBe("Pawbot unavailable · restart pawbot")
  ...
  test("replays events after asynchronous history hydration", async () => {
    ...
    const app = PawbotTui.mount(
      setup.renderer,
      { ...options, apiUrl: "http://pawbot.test", apiToken: "token", chatId: "chat" },
      client(),
      new MockTreeSitterClient({ autoResolveTimeout: 0 }),
    )
...
describe("PawbotTui with a Herdr pane title reporter", () => {
  test("keeps the full terminal experience while reporting task titles", async () => {
    ...
    expect(activeFrame).toContain(">_  pawbot")
...
if (process.platform !== "win32") {
  test("restores the terminal after SIGTERM", async () => {
    const child = Bun.spawn(["bun", "src/index.ts"], {
      ...
      env: {
        ...process.env,
        PAWBOT_TUI_WS_URL: "ws://127.0.0.1:9/ws",
        PAWBOT_TUI_API_URL: "",
        PAWBOT_TUI_API_TOKEN: "",
        PAWBOT_TUI_CHAT_ID: "resume-chat",
        PAWBOT_TUI_MODEL: "test/model",
        PAWBOT_TUI_WORKSPACE: "/tmp/pawbot-test",
