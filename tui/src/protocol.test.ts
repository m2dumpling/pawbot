...
describe("gateway protocol", () => {
  test("bootstraps fresh websocket and API credentials", async () => {
    ...
    globalThis.fetch = (async (_input: string | URL | Request, init?: RequestInit) => {
      ...
      return new Response(JSON.stringify({
        ws_url: "ws://pawbot.test/ws?mode=local",
        token: "socket token",
        api_token: "api-token",
      }))
    }) as typeof fetch
    ...
    try {
      const connection = await fetchGatewayConnection(
        "http://pawbot.test/webui/bootstrap",
        "bootstrap-secret",
        "http://pawbot.test",
        "tui-42",
      )
      expect(headers?.get("X-Pawbot-Auth")).toBe("bootstrap-secret")
      expect(connection).toEqual({
        wsUrl: "ws://pawbot.test/ws?mode=local&token=socket+token&client_id=tui-42",
    ...
    try {
      await expect(fetchSlashCommands(
        ...
        async (rejectedApiToken) => {
          ...
          return { apiUrl: "http://pawbot.test", apiToken: "fresh-api-token" }
        },
  ...
  test("waits for bootstrap before opening the websocket", async () => {
    ...
    try {
      ...
      resolveConnection?.({
        wsUrl: "ws://pawbot.test/ws?token=fresh",
        apiUrl: "http://pawbot.test",
        apiToken: "fresh-api-token",
      })
      await Bun.sleep(1)
      expect(requestedUrl).toBe("ws://pawbot.test/ws?token=fresh")
    ...
    try {
      const client = new PawbotClient({
        resolveConnection: async () => {
          ...
          return {
            wsUrl: "ws://pawbot.test/ws?token=second",
            apiUrl: "http://pawbot.test",
      ...
      expect(requestedUrl).toBe("ws://pawbot.test/ws?token=second")
      client.close()
    } finally {
      Object.defineProperty(globalThis, "WebSocket", { configurable: true, value: original })
    }
  ...
  test("loads runtime model and access controls from canonical APIs", async () => {
    ...
    try {
      expect(await fetchRuntimeControls("http://pawbot.test", "secret")).toEqual({
  ...
  test("reattaches the same generated chat after a transient disconnect", async () => {
    ...
    try {
      ...
      expect(reconnecting).toMatchObject({
        ...
        info: { endpoint: "pawbot.test", attempt: 1 },
      })
      sockets[1]?.emit("open")
  ...
  test("reports when the bounded history snapshot omits earlier turns", async () => {
    ...
    try {
      const history = await fetchHistory("http://pawbot.test", "token", "chat", "newer-page")
  ...
  test("loads the explainable session context projection", async () => {
    ...
    try {
      expect(await fetchSessionContext("http://pawbot.test", "secret", "chat")).toEqual({
        totalMessages: 24,
        archivedMessages: 16,
        replayMessages: 10,
  ...
  test("loads the gateway-owned slash command catalog", async () => {
    ...
    try {
      expect(await fetchSlashCommands("http://pawbot.test", "secret")).toEqual([{
        command: "/history",
        title: "History",
        description: "Show recent messages",
        argHint: "[n]",
  ...
  test("loads only enabled and available skills", async () => {
    ...
    try {
      expect(await fetchAvailableSkills("http://pawbot.test", "secret")).toEqual([{
        name: "verify",
        description: "Verify public behavior",
        source: "builtin",
      }])
  ...
  test("loads and normalizes WebUI sessions", async () => {
    ...
    try {
      expect(await fetchSessions("http://pawbot.test", "secret")).toEqual([{
        chatId: "chat-1",
  ...
  test("combines installed tools and saved sessions in one mention namespace", async () => {
    ...
    try {
      expect(await fetchMentionCandidates("http://pawbot.test", "secret")).toEqual([
        {
          kind: "cli",
          name: "github",
          displayName: "GitHub",
