...
describe("SessionMenu", () => {
  ...
  test("shows compact workspace names only when they distinguish sessions", async () => {
    ...
    await setup.renderOnce()
    expect(setup.captureCharFrame()).not.toContain("pawbot · Codex")
    ...
    await setup.renderOnce()
    const frame = setup.captureCharFrame()
    expect(frame).toContain("pawbot · Codex")
    ...
    menu.open([
      {
        ...scoped[0]!,
        workspaceScope: {
          project_path: "/work/frontend/pawbot",
          project_name: "pawbot",
          access_mode: "restricted",
        },
      },
      {
        ...scoped[1]!,
        workspaceScope: {
          project_path: "/work/backend/pawbot",
          project_name: "pawbot",
          access_mode: "restricted",
        },
      },
    ...
    expect(duplicates).toContain("frontend/pawbot")
    expect(duplicates).toContain("backend/pawbot")
  })
