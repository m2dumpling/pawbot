...
describe("tool renderers", () => {
  ...
  test("uses stable task language for common file and web tools", () => {
    expect(renderToolEvent({ phase: "end", name: "read_file", arguments: { path: "README.md" } }))
      .toBe("  ✓ Read  README.md")
    expect(renderToolEvent({ phase: "start", name: "web_search", arguments: { query: "pawbot" } }))
      .toBe("  › Search web  pawbot")
  ...
  test("shortens file paths relative to the workspace while preserving the useful tail", () => {
    const workspace = String.raw`C:\workspace\pawbot`
    expect(renderToolEvent({
      phase: "end",
      name: "read_file",
      arguments: { path: String.raw`C:\workspace\pawbot\tui\src\app.ts` },
    }, { workspace })).toBe("  ✓ Read  tui/src/app.ts")
    expect(renderToolEvent({
      phase: "end",
      name: "read_file",
      arguments: {
        path: String.raw`C:\workspace\pawbot\.worktrees\feature\pawbot\providers\fallback_provider.py`,
      },
    }, { workspace })).toBe("  ✓ Read  …/feature/pawbot/providers/fallback_provider.py")
  })
