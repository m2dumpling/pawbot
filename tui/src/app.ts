...
const COMPOSER_PLACEHOLDER = "Woof — give me a task"
...
function connectionStatusText(
  ...
  if (status === "unavailable") {
    return info?.health === "degraded"
      ? "Still getting ready…"
      : "Pawbot is taking longer to respond…"
  }
  if (status === "error") return "Pawbot unavailable · restart pawbot"
  return "Session ended"
}

export function sessionExitMessage(chatId: string): string {
  const sessionId = `websocket:${chatId}`
  return `Resume with: pawbot agent --session ${sessionId}\n`
}
...
export class PawbotTui {
  ...
  private updateTitle(): void {
    const identity = this.sessionTitle.trim() || "pawbot"
