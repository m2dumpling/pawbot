...
# Render deploy path (see render.yaml + render-config.example.json). Gated on Render's
...
if [ "$RENDER" = "true" ]; then
    ...
    if [ ! -f "$config" ]; then
        echo "[entrypoint] initializing $config from bundled Render template"
...
# re-exec as pawbot. Fail closed: if the privilege drop cannot be performed,
# exit rather than run the agent as root.
if [ "$(id -u)" = "0" ]; then
    chown -R pawbot:pawbot "$dir" 2>/dev/null || echo "[entrypoint] warning: chown $dir failed"
    if setpriv --reuid=pawbot --regid=pawbot --init-groups true 2>/dev/null; then
        echo "[entrypoint] dropping privileges to pawbot via setpriv"
        exec setpriv --reuid=pawbot --regid=pawbot --init-groups pawbot "$@"
    fi
...
Fix (pick one):
  Host:   sudo chown -R 1000:1000 ~/.pawbot
