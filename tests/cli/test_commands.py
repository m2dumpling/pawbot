...
from pawbot.cli import commands as cli_commands
from pawbot.cli import gateway_runtime as cli_gateway_runtime
from pawbot.cli import provider as provider_commands
from pawbot.cli import terminal as cli_terminal
from pawbot.cli import webui as cli_webui
from pawbot.cli import webui_support as cli_webui_support
from pawbot.cli.commands import app
from pawbot.config.schema import Config
from pawbot.cron.service import CronJobSkippedError
from pawbot.cron.session_turns import CRON_DEFER_UNTIL_IDLE_META, CRON_TRIGGER_META
from pawbot.cron.types import CronJob, CronPayload
from pawbot.cron.webui_metadata import cron_proactive_delivery_metadata
from pawbot.providers.factory import ProviderSnapshot, make_provider, provider_signature
from pawbot.providers.openai_codex_provider import _strip_model_prefix
from pawbot.providers.registry import find_by_name
from pawbot.providers.unconfigured_provider import UnconfiguredProvider
from pawbot.session.webui_turns import WebuiTurnRoutePolicy
from pawbot.webui.dev import WebUIDevError
from pawbot.webui.metadata import (
...
def mock_paths():
    ...
    with patch("pawbot.config.loader.get_config_path") as mock_cp, \
         patch("pawbot.config.loader.save_config") as mock_sc, \
         patch("pawbot.config.loader.load_config") as mock_lc, \
         patch("pawbot.cli.commands.get_workspace_path") as mock_ws:
...
def test_onboard_fresh_install(mock_paths):
    ...
    assert "pawbot is ready" in result.stdout
...
def test_onboard_recommends_webui(mock_paths):
    ...
    assert result.exit_code == 0
    assert "✓ pawbot is ready. Run: pawbot webui" in result.stdout
...
def test_onboard_interactive_discard_does_not_save_or_create_workspace(mock_paths, monkeypatch):
    ...
    from pawbot.cli.onboard import OnboardResult
...
def test_onboard_uses_explicit_config_and_workspace_paths(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("pawbot.channels.registry.discover_all", lambda: {})
    ...
    assert f'pawbot webui -c "{resolved_config}"' in result.stdout
...
def test_plugins_list_uses_explicit_config(monkeypatch, tmp_path: Path):
    from pawbot.channels.plugin import ChannelPlugin
    ...
    monkeypatch.setattr(
        "pawbot.channels.registry.discover_plugins",
    ...
    monkeypatch.setattr(
        "pawbot.optional_features.optional_dependency_groups",
        lambda: {},
    )
...
def test_provider_logout_xai_grok_removes_instance_credentials(tmp_path, monkeypatch):
    ...
    lock_path.write_text("", encoding="utf-8")
    monkeypatch.setattr(
        "pawbot.providers.xai_oauth.get_xai_oauth_storage_path",
        lambda: token_path,
    )
...
def test_provider_logout_xai_grok_uses_explicit_config_path(tmp_path, monkeypatch):
    from pawbot.config import loader
...
def test_provider_logout_paths_resolve_to_expected_files():
    ...
    from pawbot.providers.github_copilot_provider import get_storage
    from pawbot.providers.xai_oauth import get_xai_oauth_storage_path
...
def test_provider_login_openai_codex_handles_missing_oauth_symbol(monkeypatch):
    ...
    assert result.exit_code == 1
    assert (
        "This pawbot installation is missing the required oauth-cli-kit package. "
        "Reinstall or upgrade pawbot-ai using the same installation method."
...
def test_provider_login_xai_grok_runs_browser_flow_with_configured_proxy(monkeypatch):
    ...
    monkeypatch.setattr(
        "pawbot.providers.xai_oauth.get_xai_oauth_token",
    ...
    monkeypatch.setattr("pawbot.providers.xai_oauth.login_xai_oauth", fake_login)
...
def test_github_copilot_provider_strips_prefixed_model_name():
    from pawbot.providers.github_copilot_provider import GitHubCopilotProvider
...
async def test_github_copilot_provider_refreshes_client_api_key_before_chat():
    ...
    with patch("pawbot.providers.openai_compat_provider.AsyncOpenAI", return_value=mock_client):
...
def test_make_provider_passes_extra_headers_to_custom_provider():
    ...
    with patch("pawbot.providers.openai_compat_provider.AsyncOpenAI") as mock_async_openai:
        provider = make_provider(config)
...
def mock_agent_runtime(tmp_path):
    ...
    with patch("pawbot.config.loader.load_config", return_value=config) as mock_load_config, \
         patch("pawbot.config.loader.resolve_config_env_vars", side_effect=lambda c: c), \
         patch("pawbot.cli.agent.sync_workspace_templates") as mock_sync_templates, \
         patch("pawbot.providers.factory.make_provider", return_value=_fake_provider()), \
         patch("pawbot.cli.terminal._print_agent_response") as mock_print_response, \
         patch("pawbot.bus.queue.MessageBus"), \
         patch("pawbot.cron.service.CronService"), \
         patch("pawbot.cli.agent.AgentLoop.from_config") as mock_from_config:
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "pawbot.config.loader.set_config_path",
        lambda path: seen.__setitem__("config_path", path),
    )
    monkeypatch.setattr("pawbot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("pawbot.cli.agent.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("pawbot.providers.factory.make_provider", lambda _config: _fake_provider())
    monkeypatch.setattr("pawbot.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("pawbot.cron.service.CronService", lambda _store: object())
    ...
    monkeypatch.setattr("pawbot.cli.agent.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("pawbot.cli.terminal._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("pawbot.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("pawbot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("pawbot.cli.agent.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("pawbot.providers.factory.make_provider", lambda _config: _fake_provider())
    monkeypatch.setattr("pawbot.bus.queue.MessageBus", lambda: object())
    ...
    monkeypatch.setattr("pawbot.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("pawbot.cli.agent.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("pawbot.cli.terminal._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_workspace_override_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("pawbot.providers.factory.make_provider", lambda _config: _fake_provider())
    monkeypatch.setattr("pawbot.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("pawbot.config.paths.get_cron_dir", lambda: legacy_dir)
...
def test_agent_custom_config_workspace_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("pawbot.cli.agent.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr(
        "pawbot.cli.terminal._print_agent_response", lambda *_args, **_kwargs: None
    )
...
def test_heartbeat_has_active_tasks(content, expected):
    from pawbot.cli.gateway_runtime import _heartbeat_has_active_tasks
...
def test_heartbeat_skips_bundled_template():
    from pawbot.cli.gateway_runtime import _heartbeat_has_active_tasks
    from pawbot.utils.helpers import load_bundled_template
...
def test_heartbeat_target_skips_archived_webui_sessions():
    from pawbot.cli.gateway_runtime import _pick_heartbeat_target_from_sessions
...
def test_heartbeat_target_uses_last_channel_for_unified_session():
    from pawbot.cli.gateway_runtime import _pick_heartbeat_target_from_sessions
    from pawbot.session.keys import LAST_CHANNEL_METADATA_KEY, UNIFIED_SESSION_KEY
...
def _patch_webui_provider_ready(monkeypatch) -> None:
    monkeypatch.setattr(
        "pawbot.providers.factory.validate_provider_setup",
        lambda _config: None,
    )


def _patch_gateway_ports_free(monkeypatch) -> None:
    monkeypatch.setattr("pawbot.cli.webui._gateway_health_ready", lambda *_a, **_kw: False)
    monkeypatch.setattr("pawbot.cli.webui._tcp_endpoint_reachable", lambda *_a, **_kw: False)
    monkeypatch.setattr("pawbot.cli.webui._webui_endpoint_reachable", lambda *_a, **_kw: False)
    monkeypatch.setattr(
        "pawbot.cli.gateway_runtime._tcp_endpoint_reachable",
        lambda *_a, **_kw: False,
    )
    monkeypatch.setattr(
        "pawbot.cli.gateway_runtime._webui_endpoint_reachable",
        lambda *_a, **_kw: False,
    )


def _record_gateway_lease_release(monkeypatch, captured: dict[str, object]) -> None:
    from pawbot.gateway import GatewayClientLease
...
def _patch_webui_managed_gateway(
    ...
    from pawbot.gateway import GatewayStatus, RuntimeResult
    ...
    monkeypatch.setattr("pawbot.gateway.GatewayRuntime", _FakeRuntime)
    monkeypatch.setattr(
        "pawbot.cli.webui._prepare_webui_bundle_for_gateway",
        lambda *_args, **_kwargs: None,
    )
    monkeypatch.setattr(
        "pawbot.cli.webui._attach_to_background_gateway",
...
def _patch_cli_command_runtime(
    ...
    *,
    ...
    monkeypatch.setattr("pawbot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("pawbot.config.loader.resolve_config_env_vars", lambda c: c)
    monkeypatch.setattr(
        "pawbot.cli.commands.sync_workspace_templates",
        sync_templates or (lambda _path: None),
    )
    monkeypatch.setattr(
        "pawbot.cli.webui.sync_workspace_templates",
        sync_templates or (lambda _path: None),
    )
    monkeypatch.setattr(
        "pawbot.cli.gateway_runtime.sync_workspace_templates",
        sync_templates or (lambda _path: None),
    )
    monkeypatch.setattr(
        "pawbot.providers.factory.make_provider",
        provider_factory,
    )
    monkeypatch.setattr(
        "pawbot.providers.factory.build_provider_snapshot",
        lambda _config: _test_provider_snapshot(provider_factory(_config), _config),
    )
    monkeypatch.setattr(
        "pawbot.providers.factory.load_provider_snapshot",
        lambda _config_path=None: _test_provider_snapshot(provider_factory(config), config),
    )
    monkeypatch.setattr(
        "pawbot.cli.webui_support._provider_setup_error",
        lambda _config: None,
    )
    _patch_gateway_ports_free(monkeypatch)

    if message_bus is not None:
        monkeypatch.setattr("pawbot.bus.queue.MessageBus", message_bus)
    if session_manager is not None:
        monkeypatch.setattr("pawbot.session.manager.SessionManager", session_manager)
    if cron_service is not None:
        monkeypatch.setattr("pawbot.cron.service.CronService", cron_service)
    if get_cron_dir is not None:
        monkeypatch.setattr("pawbot.config.paths.get_cron_dir", get_cron_dir)
...
def test_heartbeat_empty_response_still_retains_recent_messages(
    ...
    monkeypatch.setattr("pawbot.cli.gateway_runtime.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("pawbot.channels.manager.ChannelManager", _FakeChannelManager)
    monkeypatch.setattr("pawbot.cli.gateway_runtime.read_webui_sidebar_state", lambda: {})
    monkeypatch.setattr("pawbot.cli.gateway_runtime.evaluate_response", _unexpected_evaluator)
...
def test_webui_detach_leaves_managed_gateway_running(
    monkeypatch,
    tmp_path: Path,
) -> None:
    config_file = tmp_path / "instance" / "config.json"
    ...
    assert result.exit_code == 0
    assert "Gateway left running in the background" in result.stdout
    assert "attached_runtime" not in seen
    assert seen["lease_release_wait_for_stop"] is False


def test_webui_remote_detach_bootstraps_providerless_vps_config(
    monkeypatch,
    tmp_path: Path,
) -> None:
    config_file = tmp_path / "instance" / "config.json"
    ...
    assert "Quick access URL" in result.stdout
    assert "Gateway left running in the background" in result.stdout
...
def test_webui_host_option_enables_explicit_remote_access(
    ...
    assert re.search(r"http://<server-ip>:8899/pawbot-[^ ]+", compact_output)
...
def test_webui_background_points_to_the_single_persistent_gateway_command(
    ...
    assert "pawbot gateway --background --config" in compact_output
...
def test_webui_dev_starts_vite_sidecar_and_gateway(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("pawbot.cli.webui.run_webui_dev_server", fake_dev_server)
    _patch_webui_managed_gateway(monkeypatch, seen)
    monkeypatch.setattr(
        "pawbot.cli.webui._attach_to_background_gateway",
    ...
    monkeypatch.setattr(
        "pawbot.cli.webui._open_webui_browser",
...
def test_webui_dev_waits_for_external_gateway_via_health_endpoint(monkeypatch) -> None:
    ...
    monkeypatch.setattr("pawbot.cli.webui._gateway_health_ready", fake_health)
    monkeypatch.setattr(
        "pawbot.cli.webui._webui_endpoint_reachable",
...
def test_webui_yes_starts_first_run_without_provider_setup(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "pawbot.cli.webui._provider_setup_error",
        lambda _config: "No API key configured for provider 'custom'.",
    )
    _patch_gateway_ports_free(monkeypatch)
    monkeypatch.setattr("pawbot.cli.webui.sync_workspace_templates", lambda _path: None)
...
def test_webui_missing_runtime_env_fails_before_starting_gateway(
    ...
    config_file = tmp_path / "config.json"
    missing_env = "PAWBOT_TEST_MISSING_WEBUI_SECRET"
    ...
    assert "pawbot status --config" in result.stdout
...
def test_webui_yes_still_refuses_invalid_custom_model_setup(
    ...
    assert "pawbot onboard --wizard" in result.stdout
    assert "pawbot status --config" in result.stdout
    assert config_file.name in result.stdout
...
def test_webui_foreground_attaches_to_existing_managed_gateway(monkeypatch, tmp_path: Path) -> None:
    ...
    _patch_webui_provider_ready(monkeypatch)
    monkeypatch.setattr("pawbot.cli.webui.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("pawbot.cli.webui._gateway_health_ready", lambda *_args, **_kwargs: True)
    monkeypatch.setattr(
        "pawbot.cli.webui_support._gateway_health_ready",
        lambda *_args, **_kwargs: True,
    )
    monkeypatch.setattr("pawbot.cli.webui._webui_endpoint_reachable", lambda *_args, **_kwargs: True)
    monkeypatch.setattr(
        "pawbot.cli.webui._open_webui_browser",
...
def test_webui_foreground_does_not_claim_unmanaged_gateway(monkeypatch, tmp_path: Path) -> None:
    ...
    _patch_webui_provider_ready(monkeypatch)
    monkeypatch.setattr("pawbot.cli.webui.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("pawbot.cli.webui._gateway_health_ready", lambda *_args: True)
    monkeypatch.setattr("pawbot.cli.webui._webui_endpoint_reachable", lambda *_args: True)
    monkeypatch.setattr("pawbot.cli.webui._open_webui_browser", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(
        "pawbot.cli.webui._attach_to_background_gateway",
...
def test_webui_foreground_refuses_occupied_webui_port(monkeypatch, tmp_path: Path) -> None:
    ...
    _patch_webui_provider_ready(monkeypatch)
    monkeypatch.setattr("pawbot.cli.webui.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("pawbot.cli.webui._gateway_health_ready", lambda *_args, **_kwargs: False)
    monkeypatch.setattr(
        "pawbot.cli.webui_support._gateway_health_ready",
        lambda *_args, **_kwargs: False,
    )
    monkeypatch.setattr("pawbot.cli.webui._webui_endpoint_reachable", lambda *_args, **_kwargs: True)
    monkeypatch.setattr("pawbot.cli.webui._tcp_endpoint_reachable", lambda *_args, **_kwargs: False)
    result = runner.invoke(app, ["webui", "--config", str(config_file), "--yes"])

    assert result.exit_code == 1
    assert "pawbot cannot start because one of its local ports is already in use" in result.stdout
...
def test_webui_foreground_reports_an_existing_gateway_without_leaking_secret(
    ...
    monkeypatch.setattr("pawbot.cli.webui._webui_endpoint_reachable", lambda *_args, **_kwargs: False)
...
def _patch_serve_runtime(monkeypatch, config: Config, seen: dict[str, object]) -> None:
    ...
    monkeypatch.setattr("pawbot.cli.commands.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("pawbot.api.server.create_app", _fake_create_app)
...
def test_gateway_unbound_agent_cron_is_skipped(
    ...
    monkeypatch.setattr("pawbot.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("pawbot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("pawbot.cli.gateway_runtime.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("pawbot.providers.factory.make_provider", lambda _config: provider)
    monkeypatch.setattr("pawbot.cli.webui_support._provider_setup_error", lambda _config: None)
    _patch_gateway_ports_free(monkeypatch)
    monkeypatch.setattr(
        "pawbot.providers.factory.build_provider_snapshot",
    ...
    monkeypatch.setattr("pawbot.bus.queue.MessageBus", lambda: bus)
    ...
    monkeypatch.setattr("pawbot.session.manager.SessionManager", _FakeSessionManager)
    ...
    monkeypatch.setattr("pawbot.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("pawbot.cli.gateway_runtime.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("pawbot.channels.manager.ChannelManager", _StopAfterCronSetup)
    monkeypatch.setattr(
        "pawbot.cli.gateway_runtime.evaluate_response",
...
def test_gateway_local_trigger_queue_submits_agent_turns(
    ...
    monkeypatch.setattr("pawbot.channels.manager.ChannelManager", _FakeChannelManager)
    monkeypatch.setattr(
        "pawbot.triggers.local_runner.run_local_trigger_queue",
...
def test_migrate_cron_store_moves_legacy_file(tmp_path: Path) -> None:
    ...
    from pawbot.cli.runtime_config import _migrate_cron_store
    ...
    with patch("pawbot.config.paths.get_cron_dir", return_value=legacy_dir):
...
def test_gateway_health_endpoint_binds_and_serves_expected_responses(
    ...
    monkeypatch.setattr("pawbot.cli.gateway_runtime.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("pawbot.channels.manager.ChannelManager", _FakeChannelManager)
    monkeypatch.setattr("pawbot.cron.service.CronService", _FakeCronService)
...
def test_gateway_agent_task_owns_initial_mcp_provider_close(
    ...
    monkeypatch.setattr("pawbot.cli.gateway_runtime.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("pawbot.cli.gateway_runtime.MCPProvider", _FakeMCPProvider)
    monkeypatch.setattr("pawbot.channels.manager.ChannelManager", _FakeChannelManager)
    monkeypatch.setattr("pawbot.cron.service.CronService", _FakeCronService)
    monkeypatch.setattr("asyncio.start_server", _fake_start_server)
...
def test_gateway_shutdown_event_exits_forever_runtime_tasks(
    ...
    monkeypatch.setattr("asyncio.start_server", _fake_start_server)
    monkeypatch.setattr(
        "pawbot.cli.gateway_runtime._install_gateway_shutdown_handlers",
