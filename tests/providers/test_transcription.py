...
from pawbot.audio.transcription import (
    EffectiveTranscriptionConfig,
    resolve_transcription_config,
    transcribe_audio_file,
)
from pawbot.audio.transcription_registry import (
    get_transcription_provider,
    resolve_transcription_provider,
    transcription_provider_names,
)
from pawbot.config.schema import Config
from pawbot.providers.transcription import (
...
def test_transcription_registry_lists_providers_and_aliases() -> None:
    ...
    assert siliconflow.adapter == "pawbot.providers.transcription:OpenAITranscriptionProvider"
...
async def test_transcribe_audio_file_routes_openrouter_provider(audio_file: Path) -> None:
    ...
    with patch("pawbot.providers.transcription.OpenRouterTranscriptionProvider", StubOpenRouter):
...
async def test_transcribe_audio_file_routes_xiaomi_mimo_provider(audio_file: Path) -> None:
    ...
    with patch("pawbot.providers.transcription.XiaomiMiMoTranscriptionProvider", StubXiaomiMiMo):
        result = await transcribe_audio_file(audio_file, config)
...
async def test_transcribe_audio_file_routes_assemblyai_provider(audio_file: Path) -> None:
    ...
    with patch("pawbot.providers.transcription.AssemblyAITranscriptionProvider", StubAssemblyAI):
        result = await transcribe_audio_file(audio_file, config)
