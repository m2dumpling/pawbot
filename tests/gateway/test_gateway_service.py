...
from pawbot.gateway import GatewayStartOptions
from pawbot.gateway.service import GatewayServiceInstaller, GatewayServiceOptions
...
def test_systemd_install_dry_run_renders_user_unit(tmp_path):
    ...
    result = installer.install(
        GatewayServiceOptions(
            start=GatewayStartOptions(
                ...
                config_path="/tmp/pawbot/config.json",
            ),
    ...
    assert result.path == tmp_path / ".config/systemd/user/pawbot-gateway.service"
    assert ("systemctl", "--user", "daemon-reload") in result.commands
    assert ("systemctl", "--user", "enable", "pawbot-gateway.service") in result.commands
    assert ("systemctl", "--user", "restart", "pawbot-gateway.service") in result.commands
    assert result.content is not None
    assert 'WorkingDirectory="/tmp/pawbot workspace"' in result.content
    assert 'ExecStart=/venv/bin/python -m pawbot gateway --foreground --port 18790 --verbose' in result.content
    assert '--workspace "/tmp/pawbot workspace" --config /tmp/pawbot/config.json' in result.content
...
def test_systemd_install_writes_unit_and_runs_commands(tmp_path):
    ...
    assert commands == [
        ...
        ["systemctl", "--user", "restart", "pawbot-gateway.service"],
    ]
...
def test_launchd_install_dry_run_renders_plist(tmp_path):
    ...
    result = installer.install(
        GatewayServiceOptions(
            start=GatewayStartOptions(
                port=18791,
                workspace="/Users/test/.pawbot/workspace",
                config_path="/Users/test/.pawbot/config.json",
            ),
    ...
    assert result.path == tmp_path / "Library/LaunchAgents/ai.pawbot.gateway.plist"
    assert result.content is not None
    payload = plistlib.loads(result.content.encode("utf-8"))
    assert payload["Label"] == "ai.pawbot.gateway"
    assert payload["ProgramArguments"] == [
        ...
        "18791",
        "--workspace",
        "/Users/test/.pawbot/workspace",
        "--config",
        "/Users/test/.pawbot/config.json",
    ]
...
def test_uninstall_systemd_removes_unit_and_reloads(tmp_path):
    ...
    unit = tmp_path / ".config/systemd/user/pawbot-gateway.service"
    ...
    assert commands == [
        ["systemctl", "--user", "disable", "--now", "pawbot-gateway.service"],
        ["systemctl", "--user", "daemon-reload"],
    ]
