...
from pawbot.security.network import validate_url_target
...
def test_onboard_does_not_crash_with_legacy_memory_window(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("pawbot.cli.commands.get_workspace_path", lambda _workspace=None: workspace)
...
def test_load_config_upgrades_legacy_context_defaults_from_model_capabilities(tmp_path) -> None:
    config_path = tmp_path / "config.json"
    config_path.write_text(
        json.dumps(
            {
    ...
    assert config.model_presets["flash"].context_window_tokens == 1_048_576


def test_load_config_preserves_an_explicit_smaller_context_budget(tmp_path) -> None:
    config_path = tmp_path / "config.json"
    config_path.write_text(
        json.dumps(
            {
