"""Tests for the Pawbot programmatic facade."""
...
from pawbot.utils.llm_runtime import runtime_from_provider_snapshot
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        Pawbot.from_config("/nonexistent/config.json")
...
def test_from_config_missing_env_reports_explicit_config_path(
    ...
    name = "PAWBOT_TEST_SDK_MISSING_KEY"
    ...
    with pytest.raises(ConfigLoadError) as exc_info:
        Pawbot.from_config(config_path)
...
def test_from_config_creates_instance(tmp_path):
    ...
    bot = Pawbot.from_config(config_path, workspace=workspace)
...
def test_from_config_composes_configured_mcp_outside_agent_loop(tmp_path):
    ...
    bot = Pawbot.from_config(config_path, workspace=tmp_path)
...
def test_from_config_accepts_default_model_preset(tmp_path):
    ...
    bot = Pawbot.from_config(config_path, workspace=tmp_path, model_preset="fast")
...
def test_from_config_default_path():
    ...
    with patch("pawbot.config.loader.load_config") as mock_load, \
         patch("pawbot.providers.factory.make_provider") as mock_prov:
...
async def test_run_with_hooks(tmp_path):
    from pawbot.agent.hook import AgentHook, AgentHookContext, SDKCaptureHook
    from pawbot.bus.events import OutboundMessage
...
def test_workspace_override(tmp_path):
    ...
    bot = Pawbot.from_config(config_path, workspace=custom_ws)
...
async def test_persisted_turn_callback_is_best_effort_and_reads_display_safe_session(tmp_path):
    from pawbot import SessionTurnPersisted
    from pawbot.agent.loop import AgentLoop
    from pawbot.bus.queue import MessageBus
    from pawbot.providers.base import LLMResponse
...
def test_runtime_client_does_not_expose_generic_event_subscription():
    from pawbot.sdk.clients import RuntimeClient
...
def test_import_from_top_level():
    ...
    assert pawbot.Pawbot is Pawbot
    assert pawbot.RequestContext.__name__ == "RequestContext"
    assert pawbot.RuntimeContextBlock.__name__ == "RuntimeContextBlock"
    assert pawbot.RuntimeContextProvider is not None
    assert pawbot.SessionTurnPersisted.__name__ == "SessionTurnPersisted"
    assert pawbot.RunResult is RunResult
    assert pawbot.RunStream is RunStream
    assert pawbot.SessionInfo is SessionInfo
    assert pawbot.SessionSnapshot is SessionSnapshot
    assert pawbot.StreamEvent is StreamEvent
    assert pawbot.StreamEventType is StreamEventType
    assert pawbot.STREAM_EVENT_TEXT_DELTA == STREAM_EVENT_TEXT_DELTA
    assert pawbot.STREAM_EVENT_RUN_COMPLETED == STREAM_EVENT_RUN_COMPLETED
    assert pawbot.STREAM_EVENT_TYPES == STREAM_EVENT_TYPES
...
async def test_sdk_capture_prefers_run_level_snapshot():
    from pawbot.agent.hook import AgentHookContext, AgentRunHookContext, SDKCaptureHook
    from pawbot.providers.base import ToolCallRequest
