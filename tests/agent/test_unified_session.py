...
from pawbot.agent.tools.file_state import FileStateStore
from pawbot.bus.events import InboundMessage
from pawbot.bus.queue import MessageBus
from pawbot.command.builtin import cmd_new, register_builtin_commands
from pawbot.command.router import CommandContext, CommandRouter
from pawbot.config.schema import AgentDefaults, Config
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("pawbot.agent.loop.SessionManager"), \
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        """save_config() writes 'unifiedSession' into config.json (simulates pawbot onboard)."""
        from pawbot.config.loader import save_config
...
class TestConsolidationUnaffectedByUnifiedSession:
    ...
    async def test_consolidation_skips_empty_session_for_unified_key(self):
        ...
        from pawbot.agent.memory import Consolidator, MemoryStore
