...
from pawbot.agent.goal_permission import goal_mutation_allowed, goal_mutation_permission
...
from pawbot.session.goal_state import GOAL_STATE_KEY
...
async def test_next_turn_after_llm_error_keeps_turn_boundary(tmp_path):
    from pawbot.agent.loop import AgentLoop
    from pawbot.agent.runner import _PERSISTED_MODEL_ERROR_PLACEHOLDER
    from pawbot.bus.events import InboundMessage
    from pawbot.bus.queue import MessageBus
...
async def test_subagent_max_iterations_announces_existing_fallback(tmp_path, monkeypatch):
    from pawbot.agent.subagent import SubagentManager, SubagentStatus
    from pawbot.bus.queue import MessageBus
    ...
    monkeypatch.setattr("pawbot.agent.tools.filesystem.ListDirTool.execute", fake_execute)
