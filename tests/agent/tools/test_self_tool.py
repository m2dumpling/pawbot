...
from pawbot.agent.tools.web import WebSearchConfig, WebToolsConfig
...
class TestSubagentHookStatus:
    ...
    async def test_after_iteration_updates_status(self):
        ...
        from pawbot.agent.subagent import SubagentStatus, _SubagentHook
    ...
    async def test_after_iteration_no_status_is_noop(self):
        ...
        from pawbot.agent.hook import AgentHookContext
        from pawbot.agent.subagent import _SubagentHook
...
class TestScratchpadInspection:
    ...
    async def test_inspect_scratchpad_string(self):
        tool = _make_tool()
        await tool.execute(action="set", key="current_project", value="pawbot")
        result = await tool.execute(action="check", key="current_project")
        assert "pawbot" in result
...
class TestRequestContext:
    ...
    def test_audit_reads_bound_session(self):
        ...
        with patch("pawbot.agent.tools.self.logger.info") as info:
            with request_context(ctx):
