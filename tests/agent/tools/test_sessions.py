...
from pawbot.agent.tools.sessions import ReadSessionTool, SearchSessionsTool
from pawbot.runtime_context import RuntimeContextBlock, append_runtime_context
...
async def test_search_sessions_reads_the_full_webui_transcript_after_compaction(
    ...
    monkeypatch.setattr("pawbot.webui.transcript.get_webui_dir", lambda: webui_dir)
    monkeypatch.setattr("pawbot.webui.session_list_index.get_webui_dir", lambda: webui_dir)
    manager = SessionManager(tmp_path)
    _save_session(
        manager,
        "websocket:history",
