...
from pawbot.agent.tools.search import FindFilesTool, GrepTool
...
async def test_web_search_tool_refreshes_dynamic_config_loader(monkeypatch) -> None:
    ...
    assert await tool.execute("pawbot") == "duckduckgo:pawbot:3"
...
async def test_grep_searches_xlsx_beyond_attachment_preview_limit(
    ...
    from pawbot.utils import document as document_utils
