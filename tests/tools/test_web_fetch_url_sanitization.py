...
from pawbot.agent.tools.web import WebFetchTool, _validate_url
...
def _patched_web_fetch():
    with (
        patch("pawbot.security.network.socket.getaddrinfo", _fake_resolve_public),
        patch("pawbot.agent.tools.web.httpx.AsyncClient", FakeClient),
        patch("pawbot.agent.tools.web._pinned_dns_transport", lambda: object()),
    ):
        yield
