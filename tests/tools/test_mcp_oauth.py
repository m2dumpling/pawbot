...
def _use_data_dir(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("pawbot.agent.tools.mcp_oauth.get_data_dir", lambda: tmp_path)
...
async def test_create_mcp_oauth_auth_uses_browser_handlers_and_persists_redirect(
    ...
    assert auth.context.client_metadata.client_uri is None
    assert auth.context.client_metadata.logo_uri is None
...
async def test_official_mcp_sdk_completes_discovery_registration_and_token_exchange(
    ...
    async def respond(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path == "/register":
            ...
            assert "client_uri" not in registration
            assert "logo_uri" not in registration
            return httpx.Response(201, json={
                "client_id": "pawbot-client",
                "redirect_uris": ["https://agent.example/auth/mcp/callback"],
