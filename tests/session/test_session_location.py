...
from pawbot.session.manager import JsonlSessionStore, SessionManager
...
def test_sessions_are_stored_outside_workspace(tmp_path: Path) -> None:
    ...
    workspace_id = (workspace / ".pawbot" / "workspace-id").read_text(encoding="utf-8").strip()
...
def test_workspace_identity_marker_contains_no_session_content(tmp_path: Path) -> None:
    ...
    marker = workspace / ".pawbot" / "workspace-id"
...
def test_sessions_follow_active_custom_config_data_root(
    ...
    assert not (default_home / ".pawbot" / "sessions").exists()
...
def test_copied_workspace_gets_isolated_session_identity(tmp_path: Path) -> None:
    ...
    assert (copied / ".pawbot" / "workspace-id").read_text(encoding="utf-8") != (
        original / ".pawbot" / "workspace-id"
...
def test_equivalent_workspace_paths_share_one_store(tmp_path: Path) -> None:
    ...
    try:
        ...
        # Windows requires SeCreateSymbolicLinkPrivilege unless Developer Mode
        # is enabled. The path identity behavior is covered on hosts that can
        # create links; lack of the OS capability is not a product failure.
        if getattr(exc, "winerror", None) == 1314 or isinstance(exc, NotImplementedError):
