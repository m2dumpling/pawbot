...
from pawbot.config.schema import Config, InlineFallbackConfig, ModelPresetConfig
from pawbot.llm_usage import get_llm_usage_store
from pawbot.llm_usage.models import LLMCallRecord
...
from pawbot.webui.settings_services import WebUIOAuthFlowRegistry
...
def test_settings_payload_propagates_preset_resolution_failure(
    ...
    config = Config()
    monkeypatch.setattr("pawbot.webui.settings_api.load_config", lambda: config)
    monkeypatch.setattr(
        Config,
        "resolve_preset",
...
def test_curated_model_reasoning_values_survive_gateway_provider_names() -> None:
    assert _reasoning_effort_values_for(
        "openrouter",
...
def test_settings_payload_includes_versioned_docs(
    ...
    monkeypatch.setattr("pawbot.webui.settings_api.__version__", "0.2.3")
...
def test_settings_payload_includes_relocated_capabilities(
    ...
    assert payload["observability"]["provider"] == "local"
    assert payload["observability"]["local_trace_enabled"] is True
...
def test_update_model_configuration_rolls_back_sessions_when_config_save_fails(
    ...
    monkeypatch.setattr("pawbot.webui.settings_api._save_settings_config", fail_save)
...
def test_update_agent_settings_marks_timezone_as_manual(
    ...
    monkeypatch.setattr(
        "pawbot.config.timezone.get_localzone_name",
...
def test_update_agent_settings_accepts_provider_discovered_context_window(
    tmp_path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    config_path = tmp_path / "config.json"
...
def test_settings_payload_includes_oauth_provider_status(
    ...
    monkeypatch.setattr("pawbot.webui.settings_api._oauth_provider_status", fake_oauth_status)
...
def test_xai_grok_status_accepts_refreshable_login(
    ...
    monkeypatch.setattr(
        "pawbot.providers.xai_oauth.get_xai_oauth_login_status",
        lambda: token,
    )
...
def test_openai_codex_oauth_login_passes_configured_proxy(
    ...
    monkeypatch.setattr(
        "pawbot.providers.openai_codex_oauth.start_openai_codex_oauth_login",
        fake_start,
    )
    ...
    monkeypatch.setattr(
        "pawbot.providers.openai_codex_oauth.complete_openai_codex_oauth_login",
        fake_complete,
    )
    monkeypatch.setattr(
        "pawbot.webui.settings_api.settings_payload",
...
def test_openai_codex_oauth_login_reports_missing_oauth_cli_kit(
    ...
    def fake_import(name, *args, **kwargs):
        if name == "pawbot.providers.openai_codex_oauth":
...
def test_github_copilot_oauth_login_reports_missing_oauth_cli_kit(
    ...
    def fake_import(name, *args, **kwargs):
        if name == "pawbot.providers.github_copilot_provider":
            raise ImportError("missing")
        return real_import(name, *args, **kwargs)
...
def test_xai_grok_login_starts_fresh_browser_flow_with_proxy(
    ...
    monkeypatch.setattr("pawbot.providers.xai_oauth.start_xai_oauth_login", fake_start)
    ...
    monkeypatch.setattr(
        "pawbot.providers.xai_oauth.complete_xai_oauth_login",
        fake_complete,
    )
    monkeypatch.setattr(
        "pawbot.webui.settings_api.settings_payload",
...
def test_azure_openai_spec_no_longer_requires_api_key() -> None:
    ...
    from pawbot.webui.settings_api import _provider_requires_api_key
